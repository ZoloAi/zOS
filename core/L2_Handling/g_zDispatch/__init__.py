# zOS/core/L2_Handling/g_zDispatch/__init__.py
...
in zOS, implementing a Facade pattern to orchestrate command execution with
...
Facade Pattern:
    ...
    1. Component Creation:
       ...
       - Store references to zOS instance, session, and logger
    ...
    3. Result Handling:
       ...
       - Handle mode-specific returns (zCLI vs. Bifrost)
...
Usage:
    ...
    from zOS.L2_Core import e_zDispatch as dispatch_module
    dispatch = dispatch_module.zDispatch(zos)
    
    # Import specific components
    from zOS.L2_Handling.g_zDispatch import zDispatch, handle_zDispatch
    dispatch = zDispatch(zos)
    result = handle_zDispatch("action", cmd, zos=zos)
    
    # Typical usage in zOS
    result = zos.dispatch.handle("action", {"zFunc": "my_function"})
    
    # With modifiers
    result = zos.dispatch.handle("^save", {"zFunc": "save"})  # Bounce back
    result = zos.dispatch.handle("menu*", menu_dict)          # Create menu
...
Thread Safety:
    - All components rely on thread-safe instances from zOS
...
__version__ = "1.5.4"
__author__ = "Zolo"
__description__ = "Core command dispatch and routing subsystem for zOS"
