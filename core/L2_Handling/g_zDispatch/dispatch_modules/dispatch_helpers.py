# zOS/core/L2_Handling/g_zDispatch/dispatch_modules/dispatch_helpers.py
...
Integration:
    ...
    - Session: Reads from zos.session dict (canonical source of truth)
...
def is_bifrost_mode(session: Dict[str, Any]) -> bool:
    ...
    execution context is in Bifrost mode (WebSocket/API mode) vs zCLI mode.
    
    Args:
        session: Session dictionary from zos.session
    
    Returns:
        True if session mode is "zBifrost", False otherwise (defaults to zCLI)
    ...
    Implementation Notes:
        ...
        - Gracefully handles missing zMode key (defaults to False/zCLI)
