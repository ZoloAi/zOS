# zOS/core/L2_Core/g_zParser/parser_modules/parser_utils.py

"""
Utility functions for parsing operations within zParser subsystem.

This module provides essential parsing utilities used throughout the zOS framework:
...
**zExpr_eval** is used by:
    - zOS/subsystems/zNavigation/navigation_modules/navigation_linking.py (Line 545)
...
External:
    - zOS core imports (os, json, yaml)
    - zOS typing imports (Any, Dict, List, Optional, Union)
...
from zOS import os, json, yaml, Any, Dict, List, Optional, Union

# Subsystem constant SSOT (shared/file_constants) — avoid local literal copies.
from .shared.file_constants import FILE_EXT_YAML
...
def handle_zRef(
    ...
    Args:
        ...
        display: Optional display adapter for visual feedback
                 (CLI/Bifrost mode-agnostic)
        zos: Optional zOS instance, threaded to the path-trust gate so the sealed
             zGuard policy has workspace context (no-op in open-core)
    ...
    try:
        ...
        *file_parts, final_key = parts
        ...
        # zGuard path-trust gate (single seam): zRef reads a file off disk, so it
        # must pass the same policy as parse_file_by_path / resolve_data_path.
        # No-op in open-core; raises PathTrustError (propagates) when zGuard denies.
        # zos is threaded from the facade so the sealed policy has full context
        # (workspace root / allowed roots), consistent with the other two gates.
        from .parser_trust import verify_path_trust
        verify_path_trust(yaml_path, zos, logger)
        ...
        # Load via zLoader's raw I/O SSOT, then parse YAML from the string.
        ...
        data = yaml.safe_load(load_file_raw(yaml_path, logger))

        logger.info(f"{LOG_PREFIX_LOAD} {LOG_MSG_ZREF_LOADED}", yaml_path, final_key)
        return data.get(final_key)

    except (ValueError, FileNotFoundError, yaml.YAMLError, OSError, RuntimeError) as e:
...
def semantic_keys(
    ...
    *,
    ...
    drop_underscored: bool = False,
) -> List[str]:
    """
    Return the meaningful (non-metadata) top-level keys of a parsed block.

    A parsed zOS block interleaves SEMANTIC children — the things that render,
    dispatch, or count as journey stops — with METADATA & chrome (``zMeta``,
    ``_zClass``, ``_zId``, ``_data`` …). Almost every READ-ONLY "twin" of an
    executor needs the same first move: look at a parsed block and enumerate just
    its semantic children, ignoring the noise. This is that single definition,
    so "what counts as structure vs. chrome" lives in ONE place instead of being
    re-spelled (slightly differently) by each caller.

    Consumed by:
        - the renderer's block resolution / reload parse-safety net
          (``HTTPRouter.resolve_first_block`` → first non-``zMeta`` block)
        - ``zProbe`` stop counting (wizard steps = children minus wizard chrome)

    Args:
        block: A parsed block dict. Anything else returns ``[]``.
        exclude: Iterable of exact key names to drop (e.g. ``{"zMeta"}`` for a
                 page, the wizard-chrome set for zProbe). Any container that
                 supports ``in`` works (set/frozenset/tuple).
        drop_underscored: Also drop keys beginning with ``"_"`` — the zOS private
                 / chrome convention (``_zClass``, ``_zId``, ``_data`` …).

    Returns:
        Ordered list of semantic key names (insertion order preserved). Empty
        when ``block`` is not a dict or has no semantic children.

    Examples:
        >>> semantic_keys({"zMeta": {}, "Hero": {}, "Body": {}}, exclude={"zMeta"})
        ['Hero', 'Body']
        >>> semantic_keys({"_zClass": "x", "step1": {}}, drop_underscored=True)
        ['step1']
    """
    ...
    return [
        ...
        if k not in exclude and not (drop_underscored and str(k).startswith("_"))
    ]
