# zOS/core/L2_Core/g_zParser/parser_modules/__init__.py

"""
Parser modules package - specialized parsing functionality for zOS.

This package provides comprehensive parsing capabilities for the zOS framework,
...
Module Breakdown:
    ...
    **vafile/ package** (Specialized):
        zVaFile (zVacuum File) parsing for declarative zOS files.
...
Usage Examples:
    ...
    >>> # Path operations
    >>> from zOS.L2_Handling.d_zParser.parser_modules import zPath_decoder, identify_zFile
    >>> path = zPath_decoder("zMachine.Config", logger)
    >>> file_type = identify_zFile("zUI.users.yaml")
    
    >>> # Command parsing
    >>> from zOS.L2_Handling.d_zParser.parser_modules import parse_command
    >>> cmd = parse_command("zFunc users.list --limit 10", logger)
    
    >>> # Expression evaluation
    >>> from zOS.L2_Handling.d_zParser.parser_modules import zExpr_eval
    >>> result = zExpr_eval('{"key": "value"}', logger)
    
    >>> # Plugin invocation
    >>> from zOS.L2_Handling.d_zParser.parser_modules import is_plugin_invocation, resolve_plugin_invocation
    >>> if is_plugin_invocation("&MyPlugin.do_something()"):
    ...     result = resolve_plugin_invocation("&MyPlugin.do_something()", logger, session)
    
    >>> # File parsing
    >>> from zOS.L2_Handling.d_zParser.parser_modules import parse_file_content
    >>> data = parse_file_content(raw_yaml, logger, ".yaml")
    
    >>> # zVaFile parsing
    >>> from zOS.L2_Handling.d_zParser.parser_modules import parse_ui_file
...
# Plugin Operations - Plugin syntax parsing primitives
from .parser_plugin import (
    is_plugin_invocation,       # Check if string is plugin invocation (&Plugin.func)
    parse_plugin_invocation,    # Parse &Plugin.func(args) to (plugin, func, args_str)
    parse_plugin_arguments      # Parse args_str to (args, kwargs)
)
...
__all__ = [
    ...
    # Plugin Operations (3 primitives)
    "is_plugin_invocation",        # Plugin detection: &Plugin.function
    "parse_plugin_invocation",     # Plugin syntax parsing: (name, func, args_str)
    "parse_plugin_arguments",      # Plugin argument parsing: (args, kwargs)
