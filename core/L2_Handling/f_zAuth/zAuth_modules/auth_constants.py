# zOS/core/L2_Core/d_zAuth/zAuth_modules/auth_constants.py
"""
...
SCHEMAS_DIR = "Schemas"
SCHEMA_FILE_NAME = "zSchema.auth.zolo"
...
# Context Constants (for authentication tiers and context management)
CONTEXT_ZSESSION = "zSession"
CONTEXT_APPLICATION = "application"
CONTEXT_ALL_APPS = "all_apps"
CONTEXT_ALL = "all"
CONTEXT_DUAL = "dual"
...
# zRBAC Declarative Access Control (SSOT vocabulary — shared by the wizard
# render-gate and the authoritative zData access guard). Apps declare access
# once via a `zRBAC` block; both layers evaluate it through this subsystem.
ZRBAC_KEY = "zRBAC"
ZRBAC_ZGUEST = "zGuest"
# `authenticated` is the uniform, reader-facing auth key honored everywhere
# (navbar filter, dispatch gate, dashboard, AND the wizard render-gate):
#   authenticated: true  → require_auth (signed-in only)
#   authenticated: false → zGuest       (signed-out only)
ZRBAC_AUTHENTICATED = "authenticated"
ZRBAC_REQUIRE_AUTH = "require_auth"
ZRBAC_REQUIRE_ROLE = "require_role"
# `require` is the attribute-agnostic gate: a {selector: expected} map where the
# selector resolves through the zLoom value SSOT against the live caller, e.g.
#   require: { %zloom.zVisitor.favorite_band: Pink Floyd }
#   require: { role: admin }          # bare key → %zloom.zVisitor.<key> sugar
# Every entry must match (AND); a list expected value matches by membership.
# zAuth never invents the attribute namespace — builders gate on ANY field.
ZRBAC_REQUIRE = "require"
# Deprecated: predicate gates were never wired (no producer/consumer). Detected
# only to emit a migration warning and degrade safely to require_auth.
ZRBAC_REQUIRE_PREDICATE = "require_predicate"
ZRBAC_ACTIONS = "actions"               # optional per-action / per-category override map
ZRBAC_ACTION_READ = "read"              # category bucket for read-style actions
ZRBAC_ACTION_WRITE = "write"            # category bucket for mutating actions

# zRBAC denial reasons (authoritative gate, audit logging)
_DENY_GUEST_ONLY = "Guest-only resource (already authenticated)"
_DENY_AUTH_REQUIRED = "Authentication required"
_DENY_ROLE_REQUIRED = "Role required: %s"
_DENY_ATTR_REQUIRED = "Attribute requirement not met: %s"
_DENY_GATE_UNAVAILABLE = "Access gate unavailable (zLoom resolver not ready)"
# Deprecation notice for the retired predicate gate.
_LOG_PREDICATE_DEPRECATED = (
    "[zRBAC] 'require_predicate' is deprecated and unwired — degrading to "
    "require_auth. Migrate to the attribute-agnostic 'require:' map "
    "(e.g. require: { %zloom.zVisitor.role: admin })."
)
...
# HTTP/API Constants
HTTP_MODE_KEY = "mode"
HTTP_MODE_ZCLI = "zCLI"  # Terminal mode identifier (legacy name, kept for protocol compatibility)
HTTP_DATA_KEY = "data"
...
__all__ = [
    ...
    'DEFAULT_PERSIST', 'DEFAULT_DELETE_PERSISTENT',
    'DEFAULT_IDENTITY_FIELDS', 'DEFAULT_PASSWORD_FIELD', 'DEFAULT_ROLE',

    # zRBAC Declarative Vocabulary (PUBLIC API)
    'ZRBAC_KEY', 'ZRBAC_ZGUEST', 'ZRBAC_AUTHENTICATED', 'ZRBAC_REQUIRE_AUTH', 'ZRBAC_REQUIRE_ROLE',
    'ZRBAC_REQUIRE', 'ZRBAC_REQUIRE_PREDICATE',
    'ZRBAC_ACTIONS', 'ZRBAC_ACTION_READ', 'ZRBAC_ACTION_WRITE',
    ...
    # HTTP/API (PUBLIC API)
    'HTTP_MODE_KEY', 'HTTP_MODE_ZCLI', 'HTTP_DATA_KEY',
