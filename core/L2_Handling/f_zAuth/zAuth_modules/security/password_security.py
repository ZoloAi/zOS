...
using the bcrypt algorithm. It is intentionally isolated from zOS dependencies
...
Isolation Strategy:
    ...
    - Testable in complete isolation from the zOS framework
...
Basic Usage:
    >>> from zOS.L2_Handling.f_zAuth.zAuth_modules.security.password_security import PasswordSecurity
...
class PasswordSecurity:
    ...
    Architecture:
        - Stateless operations (no instance state beyond logger)
        - Pure security module (no zOS dependencies)
    ...
    def _truncate_password(self, plain_password: str) -> bytes:
        ...
        Example:
            ...
            72
        """
        password_bytes = plain_password.encode(_ENCODING_UTF8)
        if len(password_bytes) > BCRYPT_MAX_PASSWORD_BYTES:
            if self.logger:
                self.logger.warning(f"{LOG_PREFIX} {_LOG_TRUNCATION_WARNING}")
    ...
    def hash_password(self, plain_password: str) -> str:
        ...
        if not plain_password:
            raise ValueError(_ERR_EMPTY_PASSWORD)
        ...
        return hashed.decode(_ENCODING_UTF8)
    ...
    def verify_password(self, plain_password: str, hashed_password: str) -> bool:
        ...
        try:
            ...
            return bcrypt.checkpw(
                password_bytes,
                hashed_password.encode(_ENCODING_UTF8)
            )
        except Exception as e:
            if self.logger:
                self.logger.error(f"{LOG_PREFIX} {_LOG_VERIFICATION_ERROR}: {e}")
            return False
