# zOS/core/L2_Core/c_zDisplay/zDisplay_modules/b_primitives/display_rendering_helpers.py
...
def get_config_value(display: Any, config_type: str, key: str, default: Any = None) -> Any:
    ...
    Args:
        display: Display instance (must have .zos.config attribute)
    ...
    Replaces Pattern:
        # OLD (4+ occurrences):
        if hasattr(self.display, 'zos') and hasattr(self.display.zos, 'config'):
            value = self.display.zos.config.get_machine("terminal")
    ...
    if not hasattr(display, 'zos') or not hasattr(display.zos, 'config'):
        return default

    # Access appropriate config type
    try:
        if config_type == 'machine':
            value = display.zos.config.get_machine(key)
        elif config_type == 'environment':
            value = display.zos.config.get_environment(key)
        else:
            return default
...
def wrap_with_color(content: str, color_name: str, zColors: Any) -> str:
    ...
    Args:
        ...
        zColors: Colors instance (must have RESET attribute)
    ...
    Replaces Pattern:
        # OLD (3+ occurrences):
        color_code = get_semantic_color(color_name)
        if color_code:
