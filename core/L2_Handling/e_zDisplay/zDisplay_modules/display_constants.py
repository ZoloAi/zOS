...
Centralized constants for all zDisplay events and modules in the zOS framework.
...
PUBLIC Constants (NO _ prefix) - 42 constants:
  ...
  - Modes: MODE_ZCLI, MODE_BIFROST, MODE_WALKER (4)
...
# Mode protocol values are single-sourced in core/zVocabulary.py. Import via the
# submodule path so this stays import-safe during zOS package initialization.
...
DEFAULT_MODE = ZMODE_ZCLI  # alias → zVocabulary.ZMODE_ZCLI

# Display Modes — zCLI/Bifrost are protocol (SSOT in zVocabulary); Walker/empty
# are zDisplay-local (no cross-subsystem protocol equivalent).
MODE_ZCLI = ZMODE_ZCLI        # alias → zVocabulary.ZMODE_ZCLI
MODE_BIFROST = ZMODE_ZBIFROST  # alias → zVocabulary.ZMODE_ZBIFROST
MODE_WALKER = "Walker"
MODE_EMPTY = ""

# Terminal-side modes (everything not in here is treated as Bifrost/GUI).
TERMINAL_MODES = (MODE_ZCLI, MODE_WALKER, MODE_EMPTY)

# Event Names - Output Events (Internal)
# _EVENT_NAME_* are kept for the GUI-event call sites but alias the base event
# names so each value is single-sourced (no duplicate string literals).
_EVENT_TEXT = "text"
_EVENT_RICH_TEXT = "rich_text"
_EVENT_HEADER = "header"
_EVENT_CODE = "code"
_EVENT_LINE = "line"
_EVENT_NAME_HEADER = _EVENT_HEADER
_EVENT_NAME_TEXT = _EVENT_TEXT
_EVENT_NAME_CODE = _EVENT_CODE
...
_EVENT_INFO = "info"
_EVENT_PRIMARY = "primary"
_EVENT_SECONDARY = "secondary"
_EVENT_ZMARKER = "zMarker"
_EVENT_NAME_ERROR = _EVENT_ERROR
_EVENT_NAME_WARNING = _EVENT_WARNING
_EVENT_NAME_SUCCESS = _EVENT_SUCCESS
_EVENT_NAME_INFO = _EVENT_INFO
_EVENT_NAME_ZMARKER = _EVENT_ZMARKER
...
_EVENT_NAME_LIST = _EVENT_LIST
_EVENT_NAME_JSON = _EVENT_JSON
...
_EVENT_NAME_SELECTION = _EVENT_SELECTION
_EVENT_NAME_BUTTON = _EVENT_BUTTON
...
# zDisplay read-primitive prompt (read_string/read_password/selection) over Bifrost.
# Renamed from "input_request" — it was a flipped near-twin of zGuard's
# "sandbox_input_request" (terminal stdin). Name now says which door it is.
_EVENT_TYPE_DISPLAY_PROMPT_REQUEST = "display_prompt_request"
...
# Spinner animation frames — SSOT for every animated indicator (spinner events
# AND the zProgress journey's spinner type). Keyed by the STYLE_* names above so
# callers reference one source instead of redefining frame sets.
SPINNER_FRAMES = {
    STYLE_DOTS:          ["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"],
    STYLE_LINE:          ["-", "\\", "|", "/"],
    STYLE_ARC:           ["◜", "◠", "◝", "◞", "◡", "◟"],
...
# Boolean input — accepted terminal responses (empty = use default)
_VALID_BOOL_INPUTS = {'y', 'yes', 'n', 'no', ''}

# Markers (Internal)
# Checkboxes (multi-select) and radios (single-select). Radios were previously
# both " " — the selected default was invisible; use ASCII radio glyphs that
# align with the 3-char checkbox markers.
_MARKER_CHECKED = "[*]"
_MARKER_UNCHECKED = "[ ]"
_MARKER_SELECTED = "(*)"
_MARKER_UNSELECTED = "( )"
...
_MSG_MULTI_SELECT_INSTRUCTIONS = "Use numbers separated by spaces (e.g., '1 3 5') or 'done'"
...
_PROMPT_IMAGE = "Click [Open {label}]? (y/n): "  #> shared by all media (image/video/audio) <#
_PROMPT_RUN = "Run this snippet? (y/n): "
_PROMPT_CONFIRM_DEFAULT = "{label}? (y/n): "

# ── Confirm-gate registry (SSOT) ─────────────────────────────────────────────
# One place for every zCLI y/n action-gate prompt. Each interactive event passes
# a `kind` key (baked into its own logic) + a label; confirm_gate() builds and
# reads the prompt uniformly. Add a key here to add a new gated event — never
# hand-roll a y/n read in an event handler again.
CONFIRM_TEMPLATES = {
    "button":          _PROMPT_BUTTON_TEMPLATE,
    "image":           _PROMPT_IMAGE,
    "video":           _PROMPT_IMAGE,  #> media siblings share the "Open {label}" wording <#
    "audio":           _PROMPT_IMAGE,
    "link_internal":   _PROMPT_LINK_INTERNAL,
    "link_external":   _PROMPT_LINK_EXTERNAL,
    "link_placeholder": _PROMPT_LINK_PLACEHOLDER,
    "link_anchor":     _PROMPT_LINK_ANCHOR,
    "run":             _PROMPT_RUN,
    "default":         _PROMPT_CONFIRM_DEFAULT,
}
...
_LABEL_ZMODE = "Mode"
_LABEL_ZOS_VERSION = "zOS Version"
