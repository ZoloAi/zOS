# zOS/core/L2_Handling/k_zOpen/open_modules/open_paths.py
...
Usage Example:
    ...
    from zOS.L2_Handling.k_zOpen.open_modules.open_paths import resolve_zpath, validate_zpath
...
from zOS import os, Optional, Any
...
def resolve_zpath(
    ...
    Args:
        ...
        session: zOS session dictionary containing workspace and machine context
    ...
    SSOT Note (intentional local resolver — does NOT delegate to zParser):
        The @/~ symbols are aliased from zVocabulary (single source). The
        resolution *algorithm* stays local on purpose: zParser.zPath_decoder
        is a different contract — it special-cases zVaFiles (strips the
        extension for zUI/zSchema/zConfig), requires a logger/zType, and
        raises on missing input. zOpen instead resolves a real on-disk file
        (extension preserved) and returns None on failure. Delegating would
        change behavior for plain files, so the symbol atoms are shared while
        the resolver is kept distinct.
