# zOS/core/L2_Handling/k_zOpen/open_modules/open_constants.py
...
# Canonical cross-subsystem vocabulary (root SSOT in zVocabulary). zPath symbols,
# session machine keys, file-extension atoms, and control-flow return signals are
# shared protocol — alias them here rather than re-declaring literals so they
# cannot drift from zLoader / zParser / zNavigation.
...
ZPATH_SYMBOL_WORKSPACE: str = PATH_SYMBOL_AT     # "@"  workspace-relative (SSOT: zVocabulary)
ZPATH_SYMBOL_ABSOLUTE: str = PATH_SYMBOL_TILDE   # "~"  absolute from root (SSOT: zVocabulary)
...
# Composed from zVocabulary atoms (SSOT). ".htm" has no shared atom — it is a
# zOpen-local variant and stays a literal here.
EXTENSIONS_HTML: tuple = (FILE_EXT_HTML, '.htm')
EXTENSIONS_TEXT: tuple = (
    FILE_EXT_TXT,
    FILE_EXT_MD,
    FILE_EXT_PY,
    FILE_EXT_JS,
...
RETURN_ZBACK: str = CONTROL_RETURN_ZBACK  # "zBack" — success, return to previous screen (SSOT: zVocabulary)
RETURN_STOP: str = CONTROL_RETURN_STOP    # "stop"  — failure, stop execution (SSOT: zVocabulary)
...
ZMACHINE_KEY_IDE: str = SESSION_KEY_IDE          # "ide"     (SSOT: zVocabulary)
ZMACHINE_KEY_BROWSER: str = SESSION_KEY_BROWSER  # "browser" (SSOT: zVocabulary)
...
_MSG_MODE_BLOCKED: str = "zOpen is a local (zCLI) operation and is disabled in this mode."
...
_LOG_MODE_BLOCKED: str = "zOpen blocked: local-only operation requested in mode '%s' (fail-closed)"
...
_LOG_IDE_UNRESOLVED: str = "IDE '%s' not resolved by detector allowlist; showing content instead of launching an unvalidated command"
