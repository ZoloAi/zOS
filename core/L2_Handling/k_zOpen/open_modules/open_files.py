# zOS/core/L2_Handling/k_zOpen/open_modules/open_files.py
...
Usage Example:
    from zOS.L2_Handling.k_zOpen.open_modules.open_files import open_file
...
from zOS import os, webbrowser, subprocess, Any
...
from zOS.L1_Foundation.a_zConfig.zConfig_modules.machine.detectors import get_ide_launch_command

# Path-trust seam (zGuard-sealed when installed; permissive in open-core).
from .open_trust import verify_path_trust, PathTrustError  # noqa: F401
...
def open_file(
    ...
    Args:
        ...
        session: zOS session dictionary containing machine configuration
        ...
        zos: Main zOS instance, threaded to the path-trust seam for full
             workspace context (optional; None in open-core/permissive path)

    Returns:
        "zBack" if file opened successfully, "stop" if failed or user cancelled

    Raises:
        PathTrustError: When zGuard's sealed path-trust policy denies the path
            (never raised in open-core, where the gate is permissive)
    ...
    # Path-trust gate (single door for read/launch). zGuard seals this with
    # workspace-containment / allowed-root policy; open-core is permissive.
    # PathTrustError must propagate unwrapped so a denial is always visible.
    verify_path_trust(path, zos, logger)
...
def _open_text(
    ...
    Args:
        ...
        session: zOS session dictionary containing IDE preference
        display: zDisplay instance for output and status messages
    ...
    try:
        ...
        if cmd:
            # Detector-resolved command (allowlisted + which()-validated upstream).
            ...
            # Windows: hand off to the OS default handler (no arbitrary binary
            # exec). If unavailable, degrade to safe content display.
            try:
                os.startfile(path)  # type: ignore
                logger.info(_LOG_SUCCESS_IDE, "startfile")
            except (AttributeError, OSError):
                logger.warning(_LOG_IDE_UNRESOLVED, editor)
                return _display_file_content(path, display, logger)
        else:
            # V2 hardening: the detector did not resolve `editor` to a known,
            # validated launch command. Do NOT exec a raw, unvalidated binary
            # name — degrade to safe content display instead.
            logger.warning(_LOG_IDE_UNRESOLVED, editor)
            return _display_file_content(path, display, logger)
        display.zDeclare(
