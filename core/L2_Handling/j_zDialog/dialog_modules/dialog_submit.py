# zOS/core/L2_Handling/j_zDialog/dialog_modules/dialog_submit.py
...
handling dict-based submission expressions (via zDispatch).
...
Key Functions
-------------
1. handle_submit(): Main entry point for onSubmit processing
2. _handle_dict_submit(): Processes dict-based submissions via zDispatch

Helper Functions
----------------
3. _inject_model_if_needed(): Handles complex model injection logic
4. _display_submit_return(): DRY helper for display return calls
...
Example 2 - Invalid submission type:
...
from zOS.L2_Handling.g_zDispatch import handle_zDispatch

from .dialog_context import inject_placeholders, KEY_MODEL, KEY_ZCONV
from .submit_result import coerce_submit_result, render_submit_signal
...
__all__ = ["handle_submit", "coerce_submit_result", "render_submit_signal"]
...
def _interpolate_session_values(obj: Any, session: Dict[str, Any], logger: Any) -> Any:
    ...
    Examples
    --------
    >>> session = {"zLobby": {"applications": {"zCloud": {"id": 1}}}}
...
def handle_submit(
    ...
    """
    if walker is None and zcli is None:
    ...
    if isinstance(submit_expr, dict):
        return _handle_dict_submit(submit_expr, zContext, logger, walker, zcli=zcli)
...
def _mask_passwords_in_dict(data: Any, password_fields: Optional[list] = None) -> Any:
    """
...
def _handle_dict_submit(
    ...
    try:
        ...
        # Resolve the zCLI/zOS instance from the walker (modern/legacy) or
        # fall back to the zcli argument passed directly from execute_code context.
        _zcli = (getattr(walker, 'zcli', None) or getattr(walker, 'zos', None)) if walker else zcli

        # Step 1.5: Interpolate session values (%session.* => actual session values)
        submit_dict = _interpolate_session_values(submit_dict, _zcli.session, logger)
        ...
        result = handle_zDispatch(_DISPATCH_CMD_SUBMIT, submit_dict, zos=_zcli, walker=walker, context=zContext)

        # Step 3.5: SSOT submit feedback (zCLI). zData returns a bare bool with
        # no signal of its own, so the ONE outcome signal is emitted here — from
        # the same ZResult Bifrost ships (see submit_result.coerce_submit_result).
        # zFunc/zLogin already surface their own ZResult (self-signal / WS reply),
        # so they are deliberately NOT re-emitted here to avoid a double signal.
        if walker is not None and isinstance(submit_dict, dict) and KEY_ZDATA in submit_dict:
            session = getattr(_zcli, "session", None)
            render_submit_signal(
                getattr(walker, "display", None),
                coerce_submit_result(submit_dict, result, session),
            )
...
def _inject_model_if_needed(
    ...
    # Case 2: zData exists => inject model into zData if missing (falls back to dialog model)
    elif KEY_ZDATA in submit_dict and isinstance(submit_dict[KEY_ZDATA], dict):
        if KEY_MODEL not in submit_dict[KEY_ZDATA]:
            submit_dict[KEY_ZDATA][KEY_MODEL] = model

    # Case 3: Neither zCRUD nor zData => inject at root level
    # Skip injection for implicit wizard sequences (all values are dicts/lists)
    # to avoid CRUD handler falsely detecting a model-only dict as a read command.
    elif KEY_MODEL not in submit_dict:
        all_nested = all(isinstance(v, (dict, list)) for v in submit_dict.values())
        if not all_nested:
...
def _display_submit_return(walker: Any) -> None:
    ...
    This DRY helper displays the standardized return feedback for submissions:
    ...
    Notes
    -----
    - Extracted to avoid duplication in submission handlers
