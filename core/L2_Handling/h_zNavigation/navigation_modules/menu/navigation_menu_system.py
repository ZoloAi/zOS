# zOS/core/L2_Handling/h_zNavigation/navigation_modules/navigation_menu_system.py
...
2. **MenuRenderer** (navigation_menu_renderer.py)
   ...
   - Supports mode-agnostic output (zCLI/Bifrost)
...
class MenuSystem:
    ...
    def create(
        ...
        **Delta Link Support (v1.5.9):**
        ...
        if mode == ZMODE_ZBIFROST:
            # Bifrost mode: emit a buffered zMenu event; the frontend's zdisplay_orchestrator
            # renders the interactive menu widget from the chunk data's ~* key.
            # The OrganizationalHandler's early-exit on *-keys prevents option branches
            # from executing on page load — option execution happens after user selection.
            self.logger.debug("[MenuSystem] Bifrost mode — emitting zMenu event (non-blocking)")
            menu_data = {
            ...
            if hasattr(self.zos, 'display') and hasattr(self.zos.display, 'primitives'):
                self.zos.display.primitives.send_gui_event('zMenu', menu_data)
        ...
        # ════════════════════════════════════════════════════════════
        # Render menu — in zCLI mode, display.zMenu() blocks for input and returns selection
        render_result = self.renderer.render(menu_obj, display)
        if render_result is not None:
            return render_result

        # Fallback: get user interaction separately (legacy path, Bifrost never reaches here)
    ...
    def _get_display(self, walker: Optional[Any]) -> Any:
        ...
        -------
        Any
            Display adapter instance (walker.display or self.zos.display)
        ...
        self.zos.display. This ensures consistent display access across all methods.
        """
