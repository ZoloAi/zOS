# zOS/core/L2_Handling/h_zNavigation/navigation_modules/navigation_menu_interaction.py
...
class MenuInteraction:
    ...
    def __init__(self, menu: Any) -> None:
        ...
        # Initialize search component (extracted)
        self.search = MenuSearch(menu)
    ...
    def get_choice_from_list(
        ...
        **Delta Link Support (v1.5.9):**
        ...
        if isinstance(selected, dict) and len(selected) == 1:
            ...
            # Explicit zLink override (zBrand or any item carrying a zLink):
            # dispatch its absolute zLink directly, bypassing structure-by-name.
            # A parent with zSub still opens its submenu (handled below).
            if (isinstance(item_data, dict) and item_data.get("zLink")
                    and "zSub" not in item_data):
                return {"zLink": item_data["zLink"]}

            # Check if this item has sub-items → open submenu and resolve child.
            if isinstance(item_data, dict) and "zSub" in item_data:
                sub_result = self.resolve_zsub(item_name, item_data)
                # zBack in the submenu → re-show THIS parent menu (recurse).
                if sub_result == NAV_ZBACK:
                    self.logger.debug("[MenuInteraction] zBack in sub-menu — re-showing parent")
                    return self.get_choice_from_list(options, display)
                return sub_result
        ...
        # Check for cross-file zLink shorthand:
        #   @.UIs.zUI.target_file           → {"zLink": "@.UIs.zUI.target_file"}
        #   zLink(@.UIs.zUI.target_file)    → {"zLink": "@.UIs.zUI.target_file"}
        if isinstance(selected, str) and selected.startswith(_PREFIX_DEFAULT_PATH):
            self.logger.debug(f"[MenuInteraction] Cross-file zLink: {selected}")
            return {"zLink": selected}

        if isinstance(selected, str) and selected.startswith(_PARSE_PREFIX_ZLINK) and selected.endswith(")"):
            path = selected[len(_PARSE_PREFIX_ZLINK):-1].strip()
            self.logger.debug(f"[MenuInteraction] zLink(...) shorthand: {path}")
            return {"zLink": path}
    ...
    def resolve_zsub(
        ...
        """Open a zSub submenu and resolve the chosen child to a navigation target.

        Shared by both menu input paths (the renderer's full-menu selection and
        get_choice_from_list) so navbar dropdowns behave identically regardless of
        which path captured the parent pick.

        Child resolution (per the zSub grammar):
            override   — child carries an explicit zLink → navigate anywhere,
                         any block (structure-by-name is bypassed)
            structural — fall back to the convention
                         ``@.zViews.{parent}.zUI.{child}.{child}``
        (Resolution itself lives in NavbarHandler.resolve_zsub_child_zlink —
        the single authority shared with the Bifrost dropdown builder.)

        Returns:
            {"zLink": <path>} for a chosen child, or NAV_ZBACK if the user backed
            out of the submenu (caller re-shows the parent).
        """
        sub_items = item_data.get("zSub")
        # zSub is normalized to {child: {zLink?, zRBAC?}} by the handler, but
        # tolerate a raw list (all-structural) for resilience.
        sub_map = (
            sub_items if isinstance(sub_items, dict)
            else {s: {} for s in sub_items if isinstance(s, str)}
        )

        display_name = item_name.lstrip('$')  # Strip $ delta prefix for display
        self.logger.debug(
            f"[MenuInteraction] Sub-menu for '{display_name}' "
            f"with {len(sub_map)} items"
        )

        # allow_back=True auto-adds the zBack option to the submenu.
        sub_choice = self.menu.zos.navigation.create(
        ...
        # SSOT: child → zLink resolved by the navbar authority (override wins,
        # else the @.zViews.{parent}.zUI.{child}.{child} convention). Bifrost's
        # dropdown builder calls the SAME helper, so the two surfaces can't drift.
        from ..handlers.handler_navbar import NavbarHandler
        child_meta = sub_map.get(sub_choice) or {}
        zlink_path = NavbarHandler.resolve_zsub_child_zlink(
            display_name, sub_choice, child_meta
        )
    ...
    def get_choice_with_search(
        ...
        Delegates to MenuSearch for all search/filter operations.
        ...
        Notes
        -----
        - Delegates to self.search.search_and_select()
        - See MenuSearch for full implementation details
        """
        return self.search.search_and_select(
            options,
            display,
            search_prompt,
            format_option_callback=self._format_option_item,
            validate_digit_callback=self._validate_digit_input,
            validate_range_callback=self._validate_index_range
        )
    ...
    def _transform_delta_link(self, selected: str) -> Dict[str, Any]:
        ...
        if zVaFolder and zVaFile:
            ...
            if zVaFolder.startswith(zpath.SIGIL_WORKSPACE):
                # Folder already carries the workspace sigil (e.g. "@.zViews" / "@")
