...
Buffers log messages in memory before zCLI framework is initialized, then
flushes them into zcli-framework.log once the framework logger is available.
...
from .constants import TS_FORMAT_TIME_MS

SESSION_LEVEL = ensure_session_level()


class BootstrapLogger:
    """
    Pre-boot logger using in-memory buffering.
    ...
    def __init__(self, name: str = "zCLI.bootstrap"):
        """Initialize bootstrap logger with in-memory buffering."""
    ...
    def session(self, msg: str, *args):
        ...
        self.logger.log(SESSION_LEVEL, msg, *args)
    ...
    def flush_to_framework(self, logger_config, verbose: bool = False):
        ...
        # Write injection header to session framework log (simplified format)
        logger_config.session_framework.info(
            "--- Bootstrap Pre-boot Logs (%d messages, %.3fs) ---",
            len(self.buffered_records),
            elapsed,
        )
        ...
        for record in self.buffered_records:
            ...
            timestamp = datetime.fromtimestamp(record.created).strftime(TS_FORMAT_TIME_MS)[:-3]
        ...
        # Write injection footer (simplified format)
        logger_config.session_framework.info(
            "--- Bootstrap Injection Complete (%.3fs total) ---",
            elapsed,
        )

        # If verbose, show completion
        if verbose:
            print(f"[Bootstrap] [ok] Initialized in {elapsed:.3f}s\n")
    ...
    def emergency_dump(self, exception: Optional[Exception] = None):
        ...
        """
        print("\n" + "=" * 70, file=sys.stderr)
        print("❌ CRITICAL: zOS Framework Initialization Failed", file=sys.stderr)
