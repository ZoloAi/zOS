...
def print_ready_message(
    ...
    Args:
        ...
        is_production: If True, suppresses output. If None (default), resolved
            from the live zSpark deployment mode (see below).
        is_testing: If True, suppresses output. If None (default), resolved
            from the live zSpark deployment mode.

    Deployment resolution:
        When neither flag is passed, the deployment mode is read from the live
        zSpark via the deployment SSOT (``is_production_from_zspark`` /
        ``is_testing_from_zspark``) so banners actually suppress in Production /
        Testing without every caller re-deriving it. Callers may still pass the
        flags explicitly to override (e.g. a ``--verbose`` force-show path).
    ...
    """
    # Resolve deployment from the live zSpark when the caller didn't say.
    # Lazy imports — Layer-0 module must not import zOS/logger at top level.
    if is_production is None and is_testing is None:
        try:
            from zOS.zCLI import get_current_zos
            from zSys.logger.config import (
                is_production_from_zspark,
                is_testing_from_zspark,
            )
            zos = get_current_zos()
            zspark = getattr(zos, "zspark_obj", None) if zos else None
            if zspark:
                is_production = is_production_from_zspark(zspark)
                is_testing = is_testing_from_zspark(zspark)
        except Exception:
            # Never let banner suppression break a subsystem's init.
            pass
