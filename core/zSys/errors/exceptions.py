...
class zCLIException(Exception):
    ...
    def __init__(self, message: str, hint: Optional[str] = None, context: Optional[dict] = None):
...
class SchemaNotFoundError(zCLIException):
    ...
    def __init__(self, schema_name: str, context_type: str = "python", zpath: Optional[str] = None):
...
class FormModelNotFoundError(zCLIException):
    ...
    def __init__(self, model_name: str, schema_name: Optional[str] = None, available_models: Optional[list] = None):
...
class InvalidzPathError(zCLIException):
    ...
    def __init__(self, zpath: str, reason: Optional[str] = None):
        hint = (
            "zPath syntax:\n"
...
class DatabaseNotInitializedError(zCLIException):
    ...
    def __init__(self, operation: Optional[str] = None, table: Optional[str] = None):
...
class zUIParseError(zCLIException):
    ...
    def __init__(self, file_path: str, issue: str, suggestion: Optional[str] = None):
        hint = suggestion or (
...
class AuthenticationRequiredError(zCLIException):
    ...
    def __init__(self, resource: Optional[str] = None, required_role: Optional[str] = None, required_permission: Optional[str] = None):
...
class PermissionDeniedError(zCLIException):
    ...
    def __init__(self, resource: Optional[str] = None, user: Optional[str] = None, required_role: Optional[str] = None):
        ...
        hint = (
            f"Contact an administrator to be assigned the required role"
            f"{f' ({required_role})' if required_role else ''}.\n\n"
            f"zRBAC is role-based: access is granted by the user's session role "
            f"(exact match), not per-action permissions."
        )
...
class PluginNotFoundError(zCLIException):
    ...
    def __init__(self, plugin_name: str, search_paths: Optional[list] = None):
        paths_msg = ""
        if search_paths:
...
class ValidationError(zCLIException):
    ...
    def __init__(self, field: str, value: Any, constraint: str, schema_name: Optional[str] = None):
        ...
        # The rejected value may be a secret/PII (e.g. a password being validated).
        # Never capture it raw — context flows to logs + the interactive traceback UI.
        # Keep type/length for debugging instead (E5).
    ...
    def __init__(self, zpath: str, resolved_path: str, context_type: str = "file"):
        ...
        if context_type == "file":
            hint = (
                ...
                f"   - macOS: ~/Library/Application Support/zOS/...\n"
                f"   - Linux: ~/.local/share/zOS/...\n"
                f"   - Windows: %LOCALAPPDATA%\\zOS\\...\n\n"
...
class UnsupportedOSError(zCLIException):
    ...
    def __init__(self, os_type: str, valid_types: tuple):
        hint = (
            ...
            f"   3. Report this issue: https://github.com/ZoloAi/zOS/issues\n"
