# zOS/core/L3_Abstraction/m_zData/zData_modules/shared/data_operations.py
"""
...
design pattern by delegating to individual operation handler modules.
...
- Consistent interface across backend adapters
...
3. Execute hook via zos.zfunc.handle(hook_expr, context)
...
from .operations.ddl_refresh import handle_refresh
from .operations.ddl_index import handle_create_index, handle_drop_index
from .operations.view_resolver import is_view
...
ACTION_REFRESH = "refresh"   # recompute a materialized view into its backing table
ACTION_INDEX = "index"          # add one index to an existing table
ACTION_DROP_INDEX = "drop_index"  # drop one index from an existing table

# ────────────────────────────────────────────────────────────────────────────
# Reserved Schema Keys (Excluded from table operations) — SSOT: shared/data_keys
# ────────────────────────────────────────────────────────────────────────────
from .data_keys import SCHEMA_KEY_META, SCHEMA_KEY_DB_PATH  # pylint: disable=wrong-import-position

RESERVED_META = SCHEMA_KEY_META
RESERVED_DB_PATH = SCHEMA_KEY_DB_PATH
...
class DataOperations:
    ...
    Integration Points:
        ...
        - zFunc: Executes hooks via zos.zfunc.handle()
    ...
    def __init__(self, handler: Any) -> None:
        ...
        (adapter, validator, schema, logger, zos) for use by operation handlers
    ...
    def display(self) -> Any:
        ...
        Returns:
            Any: zDisplay instance from zos.display
        ...
        Notes:
            ...
            - Returns self.zos.display (zDisplay facade)
            - Mode-agnostic (works in Terminal, Walker, Bifrost modes)
        """
        return self.zos.display
    ...
    def execute_hook(
        ...
        Hook Execution Flow:
            ...
            2. Check if zFunc is available (zos.zfunc exists)
            3. Execute hook via zos.zfunc.handle(hook_expr, context)
        ...
        try:
            ...
            result = self.zos.zfunc.handle(hook_expr, context)
            return result
    ...
    def _target_table_name(self, request: Dict[str, Any]) -> Optional[str]:
        """Best-effort single target name from a request (table > tables > model)."""
        tbl = request.get("table")
        if isinstance(tbl, str) and "," not in tbl:
            return tbl.strip()
        tbls = request.get("tables")
        if isinstance(tbls, list) and len(tbls) == 1:
            return tbls[0]
    ...
    def route_action(
        ...
        action_map = {
            ...
            ACTION_TRUNCATE: lambda: handle_truncate(request, self),
            ACTION_UPSERT: lambda: handle_upsert(request, self),
            ACTION_AGGREGATE: lambda: handle_aggregate(request, self),
            ACTION_WINDOW:    lambda: handle_window(request, self),
            ACTION_SET:       lambda: handle_set(request, self),
            ...
            ACTION_MIGRATE: lambda: handle_migrate(self, request, self.zos.display),
            ACTION_REFRESH: lambda: handle_refresh(request, self),
            ACTION_INDEX: lambda: handle_create_index(request, self),
            ACTION_DROP_INDEX: lambda: handle_drop_index(request, self),
        }

        # Views are read-only: a write that targets a view name is a schema
        # mistake, not a silent no-op — reject it with a clear pointer to the
        # base table(s). (Reads of a view are resolved in handle_read.)
        if action in (ACTION_INSERT, ACTION_UPDATE, ACTION_DELETE,
                      ACTION_UPSERT, ACTION_TRUNCATE):
            target = self._target_table_name(request)
            if target and is_view(getattr(self, "schema", {}) or {}, target):
                self.logger.error(
                    "[zData] '%s' is a view (read-only) — write to its base "
                    "table(s) instead, then read the view.", target
                )
                return RETURN_ERROR
    ...
    def ensure_tables_for_action(
        ...
        # INSERT: auto-create only the target table if missing (first-run behaviour)
        if action == ACTION_INSERT:
            return self.ensure_tables(tables if tables else None)

        # Actions that SKIP ensure_tables (do not need tables to exist).
        # REFRESH targets a view name (no physical table of its own); its
        # backing table is a normal declared table ensured at boot. INDEX /
        # DROP_INDEX operate on an existing table (the handler checks existence).
        if action not in [ACTION_LIST_TABLES, ACTION_DROP, ACTION_HEAD, ACTION_REFRESH,
                          ACTION_INDEX, ACTION_DROP_INDEX]:
            # All other actions (read, update, delete, upsert) REQUIRE ensure_tables
            return self.ensure_tables(tables if tables else None)

        # Action skips ensure_tables (list_tables, drop, head)
    ...
    def ensure_tables(
        ...
        for table_name in tables_to_check:
            ...
            # Views have no physical table to build — a virtual view is pure
            # spec, a materialized view fills a *separate* declared backing table.
            # Skip the entity itself so `create` never emits an empty CREATE.
            if is_view(self.schema, table_name):
                self.logger.debug("[zData] '%s' is a view — skipping table create", table_name)
                continue
    ...
    def insert_many(
        ...
        if not self.adapter:
            raise RuntimeError(ERR_RUNTIME_NO_ADAPTER)
        return self.adapter.insert_many(table, rows_data)
