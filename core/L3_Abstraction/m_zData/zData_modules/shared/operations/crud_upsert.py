# zOS/core/L3_Abstraction/m_zData/zData_modules/shared/operations/crud_upsert.py
"""
...
from ..data_keys import KEY_FIELDS, KEY_VALUES, KEY_TABLE, KEY_OPTIONS  # pylint: disable=wrong-import-position
_KEY_FIELDS = KEY_FIELDS
_KEY_VALUES = KEY_VALUES
_KEY_TABLE = KEY_TABLE
_KEY_CONFLICT_FIELDS = "conflict_fields"
_KEY_CONFLICT_KEY = "conflict_key"
_KEY_OPTIONS = KEY_OPTIONS
_KEY_DATA = "data"
_KEY_RETURNING = "returning"
...
_LOG_BULK_START = "[zData] Bulk upsert: %d rows into %s"
_LOG_BULK_ROW = "[zData] Bulk upsert row → id=%s"
_LOG_BULK_DONE = "[zData] Bulk upsert complete: %d rows written to %s"
...
def handle_upsert(request: Dict[str, Any], ops: Any) -> bool:
    ...
    # ============================================================
    # Phase 1.5: Bulk Path — data: [{…}, …]
    # ============================================================
    data_raw = request.get(_KEY_DATA)
    if isinstance(data_raw, list):
        returning = request.get(_KEY_RETURNING)
        conflict_key = request.get(_KEY_CONFLICT_KEY) or request.get(_KEY_CONFLICT_FIELDS)

        ops.logger.info(_LOG_BULK_START, len(data_raw), table)
        row_ids = []
        for row_dict in data_raw:
            if not isinstance(row_dict, dict):
                continue
            fields = list(row_dict.keys())
            values = list(row_dict.values())
            if conflict_key:
                cf = [conflict_key] if isinstance(conflict_key, str) else list(conflict_key)
            else:
                cf = [fields[0]] if fields else []
            row_id = ops.upsert(table, fields, values, cf)
            row_ids.append(row_id)
            ops.logger.info(_LOG_BULK_ROW, row_id)

        ops.logger.info(_LOG_BULK_DONE, len(row_ids), table)

        if returning:
            return _display_returning(table, row_ids, returning, ops)
    ...
    returning = request.get(_KEY_RETURNING)
    if returning:
        return _display_returning(table, [row_id], returning, ops)
