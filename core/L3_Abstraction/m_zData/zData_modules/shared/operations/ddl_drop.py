# zOS/core/L3_Abstraction/m_zData/zData_modules/shared/operations/ddl_drop.py
"""
...
from ..data_keys import KEY_TABLES  # pylint: disable=wrong-import-position
...
def handle_drop(request: Dict[str, Any], ops: Any) -> bool:
    ...
    # Single-target fallback — accept `table:` / `model:` like head/truncate/read,
    # so `action: drop, model: @.models…` resolves consistently (not just `tables:`).
    if not tables:
        single = extract_table_from_request(request, _OP_DROP_TABLE, ops, check_exists=False)
        if single:
            tables = [single]
