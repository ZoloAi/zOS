# zOS/core/L3_Abstraction/m_zData/zData_modules/shared/operations/ddl_head.py
"""
...
- Mode-agnostic: Uses zDisplay for zCLI + Bifrost output
...
This provides:
- **zCLI mode**: Formatted ASCII table with headers and separators
...
try:
    ...
    from .view_resolver import is_view, view_block
except ImportError:
    from helpers import extract_table_from_request
    from view_resolver import is_view, view_block
...
def _head_view(name: str, ops: Any) -> bool:
    """Describe a view for HEAD — its kind and the read it stands for."""
    block = view_block(ops.schema, name)
    materialized = bool(block.get("materialized"))
    kind = "materialized" if materialized else "virtual"

    rows = [{"Property": "kind", "Value": f"view ({kind})"}]
    if materialized:
        rows.append({"Property": "into (backing)", "Value": str(block.get("into", name))})
    for key in ("tables", "table", "joins", "where", "fields", "order", "limit"):
        if key in block:
            rows.append({"Property": key, "Value": str(block[key])})

    ops.display.zTable(
        title=_FMT_SCHEMA_HEADER % name,
        columns=["Property", "Value"],
        rows=rows,
        show_header=True,
    )
    ops.logger.info(_LOG_SUCCESS, name, len(rows))
    return True
...
def handle_head(request: Dict[str, Any], ops: Any) -> bool:
    ...
    Note:
        ...
        - zDisplay integration: Mode-agnostic output (zCLI + Bifrost)
    ...
    """
    # ============================================================
    # Phase 0: Views — no physical table, so show the SAVED READ, not columns
    # ============================================================
    # A view name fails a check_exists probe (nothing to open), so intercept it
    # first and describe the read it stands for: kind, source, filter, projection.
    _name = request.get("table")
    if not isinstance(_name, str):
        _model = request.get("model")
        _name = _model.split(".")[-1] if isinstance(_model, str) else None
    if _name and is_view(getattr(ops, "schema", {}) or {}, _name):
        return _head_view(_name, ops)
