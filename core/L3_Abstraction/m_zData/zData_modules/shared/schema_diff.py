# zOS/core/L3_Abstraction/m_zData/zData_modules/shared/schema_diff.py
"""
...
-------------
Basic schema diff:
    >>> from zOS.L3_Abstraction.m_zData.zData_modules.shared.schema_diff import diff_schemas
...
KEY_COLUMNS_RENAMED = "columns_renamed"
KEY_INDEXES_ADDED = "indexes_added"
KEY_INDEXES_DROPPED = "indexes_dropped"
...
# Column Properties
PROP_TYPE = "type"
PROP_RENAMED_FROM = "renamed_from"
...
def diff_schemas(old_schema: Dict[str, Any], new_schema: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for table_name in common_tables:
        ...
        table_changes = detect_table_modifications(old_table, new_table, table_name)
    ...
    # Single source of truth for "is there anything to migrate?" — consumed by
    # boot-time drift detection (zspark_command) and any caller that just needs a
    # yes/no without re-deriving it from change_count.
    has_changes = bool(tables_added or tables_dropped or tables_modified)
...
def detect_table_modifications(old_table: Dict[str, Any],
    ...
    old_indexes = old_table.get(KEY_INDEXES, [])
    new_indexes = new_table.get(KEY_INDEXES, [])
    ...
    columns_added, columns_dropped, columns_modified, columns_renamed = detect_column_changes(
        old_columns, new_columns
    )

    # Detect index changes
    indexes_added, indexes_dropped = detect_index_changes(
        table_name, old_indexes, new_indexes
    )
...
def detect_column_changes(old_columns: Dict[str, Any],
    ...
    Returns:
        Tuple of (columns_added, columns_dropped, columns_modified, columns_renamed)
        - columns_added: Dict of {column_name: column_def} for new columns
        - columns_dropped: List of column names that were removed
        - columns_modified: Dict of {column_name: {'old': old_def, 'new': new_def}}
        - columns_renamed: Dict of {new_name: old_name} — a new column carrying a
          `renamed_from: <old>` hint that names an existing (now-gone) column is a
          RENAME, not a drop+add. The pair is removed from added/dropped so no data
          is lost; a concurrent type change is still recorded in columns_modified.
    
    Examples:
        >>> old = {'id': {'type': 'integer'}, 'status': {'type': 'string'}}
        >>> new = {'id': {'type': 'integer'}, 'status': {'type': 'string', 'required': True},
        ...        'email': {'type': 'string'}}
    ...
    for col_name in sorted(common_columns):
        ...
        # Compare only properties known to the old definition.
        # When old comes from DB introspection (e.g. CSV headers → {type: str}),
        # extra metadata in new (rules, required, default, etc.) is not a DDL
        # change — it was never stored in the DB in the first place.
        comparable_old = {k: v for k, v in old_def.items() if k in new_def}
        comparable_new = {k: v for k, v in new_def.items() if k in old_def}
    ...
    # Rename pass: a NEW column with `renamed_from: <old>` that names a column
    # present in old (and absent from new) is a rename — reclaim it from add/drop.
    columns_renamed = {}
    for new_name in list(columns_added.keys()):
        new_def = columns_added[new_name]
        old_name = new_def.get(PROP_RENAMED_FROM) if isinstance(new_def, dict) else None
        if old_name and old_name in old_columns and old_name not in new_columns:
            columns_renamed[new_name] = old_name
            del columns_added[new_name]
            if old_name in columns_dropped:
                columns_dropped.remove(old_name)
            # A rename that ALSO changes the type → record the type change on the
            # new name so the executor renames first, then modifies.
            old_def = old_columns[old_name]
            old_t = old_def.get(PROP_TYPE) if isinstance(old_def, dict) else None
            new_t = new_def.get(PROP_TYPE) if isinstance(new_def, dict) else None
            if old_t is not None and new_t is not None and old_t != new_t:
                columns_modified[new_name] = {"old": old_def, "new": new_def}

    return columns_added, columns_dropped, columns_modified, columns_renamed

# ═══════════════════════════════════════════════════════════════════════════════
# INDEX DETECTION
# ═══════════════════════════════════════════════════════════════════════════════

def detect_index_changes(table_name: str, old_indexes: Any, new_indexes: Any) -> tuple:
    """
    Detect indexes added or dropped, compared BY RESOLVED NAME.

    Contract (set by the migration engine): ``old_indexes`` is the list of index
    NAMES that live in the database right now (from introspection); ``new_indexes``
    is the list of declared ``indexes:`` SPECS from the schema. Each declared spec
    is resolved to its canonical name via the shared namer, so a spec that already
    exists in the DB is a no-op (idempotent), a newly-declared spec is an ADD, and a
    live index no longer declared is a DROP.

    Returns:
        (indexes_added, indexes_dropped)
        - indexes_added: list of SPECS to CREATE (carry unique/fields/name)
        - indexes_dropped: list of NAMES to DROP
    """
    from .index_naming import resolve_index_name

    new_by_name = {}
    for spec in (new_indexes or []):
        name = resolve_index_name(table_name, spec)
        if name:
            new_by_name[name] = spec

    old_names = {n for n in (old_indexes or []) if isinstance(n, str)}
    new_names = set(new_by_name.keys())

    indexes_added = [new_by_name[n] for n in sorted(new_names - old_names)]
    indexes_dropped = sorted(old_names - new_names)
    return indexes_added, indexes_dropped
...
def _count_changes(tables_added: List[str], tables_dropped: List[str],
    ...
    counts = {
        ...
        KEY_COLUMNS_RENAMED: 0,
        KEY_INDEXES_ADDED: 0,
        KEY_INDEXES_DROPPED: 0,
        KEY_CONSTRAINTS_ADDED: 0,
        KEY_CONSTRAINTS_DROPPED: 0
    }
    ...
    for table_changes in tables_modified.values():
        ...
        counts[KEY_COLUMNS_RENAMED] += len(table_changes.get(KEY_COLUMNS_RENAMED, {}))
        counts[KEY_INDEXES_ADDED] += len(table_changes.get(KEY_INDEXES_ADDED, []))
        counts[KEY_INDEXES_DROPPED] += len(table_changes.get(KEY_INDEXES_DROPPED, []))
        counts[KEY_CONSTRAINTS_ADDED] += len(table_changes.get(KEY_CONSTRAINTS_ADDED, []))
        counts[KEY_CONSTRAINTS_DROPPED] += len(table_changes.get(KEY_CONSTRAINTS_DROPPED, []))
    ...
    if diff[KEY_TABLES_MODIFIED]:
        ...
        for table_name, changes in diff[KEY_TABLES_MODIFIED].items():
            ...
            if changes[KEY_COLUMNS_ADDED]:
                msg = (f"{INDENT_LEVEL_2}{INDENT_LEVEL_1}{SYMBOL_ADDED} "
                       f"{len(changes[KEY_COLUMNS_ADDED])} column(s) added")
                lines.append(msg)

            if changes[KEY_COLUMNS_DROPPED]:
                cols = ", ".join(changes[KEY_COLUMNS_DROPPED])
                msg = (f"{INDENT_LEVEL_2}{INDENT_LEVEL_1}{SYMBOL_DROPPED} "
                       f"{len(changes[KEY_COLUMNS_DROPPED])} column(s) dropped ({cols})")
                lines.append(msg)

            if changes[KEY_COLUMNS_MODIFIED]:
                msg = (f"{INDENT_LEVEL_2}{INDENT_LEVEL_1}{SYMBOL_MODIFIED} "
                       f"{len(changes[KEY_COLUMNS_MODIFIED])} column(s) modified")
                lines.append(msg)

            if changes.get(KEY_COLUMNS_RENAMED):
                pairs = ", ".join(f"{old}→{new}" for new, old in changes[KEY_COLUMNS_RENAMED].items())
                msg = (f"{INDENT_LEVEL_2}{INDENT_LEVEL_1}{SYMBOL_MODIFIED} "
                       f"{len(changes[KEY_COLUMNS_RENAMED])} column(s) renamed ({pairs})")
                lines.append(msg)

            if changes.get(KEY_INDEXES_ADDED):
                msg = (f"{INDENT_LEVEL_2}{INDENT_LEVEL_1}{SYMBOL_ADDED} "
                       f"{len(changes[KEY_INDEXES_ADDED])} index(es) added")
                lines.append(msg)

            if changes.get(KEY_INDEXES_DROPPED):
                names = ", ".join(changes[KEY_INDEXES_DROPPED])
                msg = (f"{INDENT_LEVEL_2}{INDENT_LEVEL_1}{SYMBOL_DROPPED} "
                       f"{len(changes[KEY_INDEXES_DROPPED])} index(es) dropped ({names})")
                lines.append(msg)

            if changes.get(KEY_CONSTRAINTS_ADDED):
                msg = (f"{INDENT_LEVEL_2}{INDENT_LEVEL_1}{SYMBOL_ADDED} "
                       f"{len(changes[KEY_CONSTRAINTS_ADDED])} constraint(s) added")
                lines.append(msg)

            if changes.get(KEY_CONSTRAINTS_DROPPED):
                msg = (f"{INDENT_LEVEL_2}{INDENT_LEVEL_1}{SYMBOL_DROPPED} "
                       f"{len(changes[KEY_CONSTRAINTS_DROPPED])} constraint(s) dropped")
                lines.append(msg)
