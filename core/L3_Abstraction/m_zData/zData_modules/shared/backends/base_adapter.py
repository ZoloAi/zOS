# zOS/core/L3_Abstraction/m_zData/zData_modules/shared/backends/base_adapter.py
"""
...
from zOS import Dict, List, Optional, Any, Path, Callable
...
class BaseDataAdapter(ABC):  # pylint: disable=unnecessary-pass
    ...
    **TCL - Transaction Control (3):**
    ...
    def __init__(
        ...
        # Cache for directory creations to reduce log noise
        self._ensured_directories: set = set()
        ...
        if self.logger:
            ...
            # Base path info is redundant with adapter initialization message above
    ...
    def register_schema(self, table_name: str, schema: Dict[str, Any]) -> None:
        """Make the adapter type-aware for an existing table (default: no-op).

        SQL backends (SQLite/Postgres) carry types in the engine, so they ignore
        this. The CSV adapter overrides it to record the schema for typed reads —
        the orchestrator calls it once at connect for every schema table.
        """

    # ============================================================
    # Blob storage hooks (default: SQL inline-bytes semantics)
    # ============================================================
    # A ``type: blob`` value is normalised to bytes upstream (coerce_blob). These
    # hooks translate between that canonical form and the backend's physical
    # storage. SQL backends store bytes inline (BLOB/BYTEA) and use these
    # defaults verbatim; the CSV adapter overrides them to spill to sidecar files.

    def store_blob(self, table_name: str, field: str, data: bytes) -> Any:  # pylint: disable=unused-argument
        """Return the cell value to persist for a blob field. SQL: bytes inline."""
        return data

    def load_blob(self, table_name: str, field: str, cell: Any) -> Any:
        """Wrap a stored blob cell into a canonical ``BlobRef`` (or ``None``)."""
        from ..blob import BlobRef  # pylint: disable=import-outside-toplevel
        if cell is None:
            return None
        ...
        # Unexpected non-binary cell on a SQL backend — pass through untouched.
        return cell

    def delete_blob(self, table_name: str, field: str, cell: Any) -> None:  # pylint: disable=unused-argument
        """Release backend storage for a blob cell on row delete. SQL: no-op."""
    ...
    # ============================================================
    # TCL - Savepoints (concrete no-op defaults — nested rollback points)
    # ============================================================
    # Backends that support nested transactions (SQL: SAVEPOINT) override these.
    # Flat backends inherit the no-op so callers never branch on capability.

    def savepoint(self, name: str) -> None:
        """Create a named savepoint within the current transaction. Default: no-op."""
        if self.logger:
            self.logger.debug("savepoint(%s) unsupported by %s — no-op", name, type(self).__name__)

    def release_savepoint(self, name: str) -> None:
        """Release (merge) a named savepoint — its work stays. Default: no-op."""
        if self.logger:
            self.logger.debug("release_savepoint(%s) unsupported by %s — no-op", name, type(self).__name__)

    def rollback_to_savepoint(self, name: str) -> None:
        """Undo work since a named savepoint, keeping the outer transaction. Default: no-op."""
        if self.logger:
            self.logger.debug("rollback_to_savepoint(%s) unsupported by %s — no-op", name, type(self).__name__)

    # ── Standalone index DDL (add/drop on an EXISTING table) ─────────────────────
    # Declared `indexes:` are built at create; these two let you add or drop an
    # index later. Backends with no index concept (CSV) inherit the no-op so a
    # caller never has to branch on capability — the same schema still runs.

    def create_index(self, table_name: str, index: Any) -> Any:
        """Create one index on an existing table. Default: no-op (returns None)."""
        if self.logger:
            self.logger.debug("create_index(%s, %s) unsupported by %s — no-op",
                              table_name, index, type(self).__name__)
        return None

    def drop_index(self, table_name: str, index: Any) -> Any:
        """Drop one index by name or field(s). Default: no-op (returns None)."""
        if self.logger:
            self.logger.debug("drop_index(%s, %s) unsupported by %s — no-op",
                              table_name, index, type(self).__name__)
        return None

    def introspect_indexes(self, table_name: str) -> list:
        """Return live user-created index names on a table. Default: none (CSV)."""
        return []

    def add_constraint(self, table_name: str, constraint: Any) -> Any:
        """Add one fk/check constraint to an existing table. Default: no-op (CSV)."""
        if self.logger:
            self.logger.debug("add_constraint(%s, %s) unsupported by %s — no-op",
                              table_name, constraint, type(self).__name__)
        return None

    def drop_constraint(self, table_name: str, constraint: Any) -> Any:
        """Drop one constraint by name/def from an existing table. Default: no-op (CSV)."""
        if self.logger:
            self.logger.debug("drop_constraint(%s, %s) unsupported by %s — no-op",
                              table_name, constraint, type(self).__name__)
        return None
    ...
    def _ensure_directory(self, path: Optional[Path] = None) -> None:
        ...
        # Only log the first time a directory is created
        if self.logger and str(target_path) not in self._ensured_directories:
            self.logger.debug(_LOG_DIRECTORY_CREATED, target_path)
            self._ensured_directories.add(str(target_path))
