# zOS/core/L3_Abstraction/m_zData/zData_modules/shared/backends/sql_adapter.py
"""
...
from ..operators import (
    normalize_operator,
    OP_LIKE, OP_IN, OP_NIN, OP_NULL, OP_NOTNULL, OP_OR,
...
from ..validators.constants import (  # pylint: disable=wrong-import-position
    ...
    SCHEMA_KEY_DEFAULT as _SCHEMA_KEY_DEFAULT,
)
...
class SQLAdapter(BaseDataAdapter):
    ...
    **6. Index Creation:**
    ...
    def create_table(self, table_name: str, schema: Dict[str, Any]) -> None:
        ...
        if _SCHEMA_KEY_PRIMARY_KEY in schema:
            ...
            if isinstance(pk_value, list) and len(pk_value) > 0:
                ...
                self._log('info', _LOG_COMPOSITE_PK, composite_pk)
        ...
        for field_name, attrs in schema.items():
            ...
            # Map type
            field_type = self._map_field_type(attrs.get(_SCHEMA_KEY_TYPE, "str"))
            ...
            if attrs.get(_SCHEMA_KEY_UNIQUE):
                column += f" {_CONSTRAINT_UNIQUE}"
            if attrs.get(_SCHEMA_KEY_REQUIRED) is True:
                column += f" {_CONSTRAINT_NOT_NULL}"

            field_defs.append(column)

            # Handle foreign keys
            if _SCHEMA_KEY_FK in attrs:
        ...
        if composite_pk:
            ...
            self._log('info', _LOG_ADD_COMPOSITE_PK, pk_columns)

        # Build and execute DDL
        all_defs = field_defs + table_constraints + foreign_keys
        ddl = f"{_SQL_CREATE} TABLE {table_name} ({', '.join(all_defs)});"
        ...
        if _SCHEMA_KEY_INDEXES in schema:
            self._create_indexes(table_name, schema[_SCHEMA_KEY_INDEXES])
    ...
    def alter_table(self, table_name, changes):
        """Alter table structure (rename/add/drop/modify columns)."""
        cur = self.get_cursor()

        # Handle RENAME COLUMN first — so any later add/drop/modify in the same
        # change set references the NEW column names. Rename preserves the data.
        if changes.get("rename_columns"):
            for new_name, old_name in changes["rename_columns"].items():
                ...
                cur.execute(sql)
            self.connection.commit()
            self._log('info', "Altered table (rename columns): %s", table_name)
    ...
    def _format_default_sql(self, value):
        """Render a column DEFAULT as a valid SQL literal.

        Strings/dates must be single-quoted (bare words read as column references
        on strict dialects like Postgres → "cannot use column reference in DEFAULT
        expression"). Numbers pass through; booleans become TRUE/FALSE.
        """
    ...
    def _build_add_column_sql(self, table_name, column_name, column_def):
        ...
        # Add DEFAULT if specified (quoted as a literal so strings aren't parsed
        # as column references on strict dialects).
        if column_def.get("default") is not None:
            sql += f" DEFAULT {self._format_default_sql(column_def.get('default'))}"

        return sql

    def _build_column_definition(self, column_name: str, column_def: Dict[str, Any]) -> str:
        """
        Build column definition string for CREATE/ALTER TABLE.
        ...
        """
        field_type = self._map_field_type(column_def.get(_SCHEMA_KEY_TYPE, "str"))
        col_def = f"{column_name} {field_type}"

        if column_def.get("primary_key") or column_def.get(_SCHEMA_KEY_PK):
            col_def += f" {_CONSTRAINT_PRIMARY_KEY}"

        if column_def.get(_SCHEMA_KEY_UNIQUE):
            col_def += f" {_CONSTRAINT_UNIQUE}"
    ...
    def _build_modify_column_sql(self, table_name, column_name, new_def):
        ...
        # The diff engine hands modify specs as {"old": {...}, "new": {...}}; the
        # actual target definition is under "new". Unwrap it (tolerating a flat def)
        # so the column is rebuilt with the intended type — not the "str" fallback.
    ...
    def insert_many(self, table, rows_data):
        """
        Insert multiple pre-processed row dicts in a single write pass.

        Mirrors the CSV adapter's ``insert_many`` so INSERT … SELECT and bulk insert
        behave identically on every backend. Each row may carry a different set of
        columns; ids are collected in order and the batch commits once.
        """
        if not rows_data:
            return []
        cur = self.get_cursor()
        ...
        for row in rows_data:
            ...
            sql = f"INSERT INTO {table} ({', '.join(fields)}) VALUES ({placeholders})"
            cur.execute(sql, values)
            row_ids.append(self._get_last_insert_id(cur))
        self._maybe_commit()
        self._log('info', "Inserted %d row(s) into %s (bulk)", len(row_ids), table)
        return row_ids
    ...
    def select(self, table, fields=None, **kwargs):
        ...
        sql = f"SELECT {distinct_kw}{select_clause} FROM {from_clause}"
        params = []
    ...
    def delete(self, table: str, where: Dict[str, Any]) -> int:
        ...
        sql = f"{_SQL_DELETE} {_SQL_FROM} {table}"
        params = []
        ...
        rows_affected = cur.rowcount
        self._log('info', _LOG_DELETE_ROWS, rows_affected, table)
        return rows_affected

    def truncate(self, table: str) -> int:
        """
        Remove every row from a table and (best-effort) reset its PK sequence.

        Emitted as an unqualified ``DELETE FROM`` — the portable clear that works
        on every SQL backend. On SQLite the AUTOINCREMENT counter also lives in
        ``sqlite_sequence``; clearing that row makes the next insert start at 1
        again. Dialects without that table simply skip the reset. Honors an open
        transaction via ``_maybe_commit`` so it composes inside a wizard.
        """
        cur = self.get_cursor()
        cur.execute(f"{_SQL_DELETE} {_SQL_FROM} {table}")
        count = cur.rowcount
        try:  # SQLite autoincrement reset — no-op / absent on other dialects
            cur.execute("DELETE FROM sqlite_sequence WHERE name = ?", (table,))
        except Exception:  # pylint: disable=broad-except
            pass
        self._maybe_commit()
        self._log('info', _LOG_DELETE_ROWS, count, table)
        return count
    ...
    def upsert(
        ...
        if not conflict_fields:
            raise ValueError(_ERR_UPSERT_MISSING_CONFLICT)
        ...
        row_id = self._get_last_insert_id(cur)
        self._log('info', _LOG_UPSERT_ROW, table, row_id)
        return row_id
        ...
        try:
            ...
            if group_by:
                ...
                log_msg = (
                    ...
                    f"grouped by {group_by}: {len(result)} groups"
                )
    ...
    def begin_transaction(self) -> None:
        """Begin transaction and suppress per-operation autocommit until commit/rollback."""
        if self.connection:
    ...
    def commit(self):
        """Commit current transaction and re-enable per-operation autocommit."""
        if self.connection:
            self.connection.commit()
    ...
    def rollback(self):
        """Rollback current transaction and re-enable per-operation autocommit."""
        if self.connection:
    ...
    def _maybe_commit(self) -> None:
        """Commit a single write only when NO explicit transaction is open.

        Each DML method calls this instead of ``connection.commit()`` directly.
        Outside a transaction this preserves the original autocommit-per-write
        behavior; inside ``begin_transaction`` it becomes a no-op so the whole
        wizard (and any nested SAVEPOINT) stays atomic until the final commit.
        """
    ...
    # -- Savepoints (nested rollback points within the open transaction) -------
    # SQLite and PostgreSQL both speak SAVEPOINT / RELEASE / ROLLBACK TO natively,
    # on the SAME connection the transaction already holds — so an inner failure
    # unwinds to the savepoint without aborting the outer transaction.

    @staticmethod
    def _safe_savepoint(name: str) -> str:
        """Coerce a step name into a SQL-identifier-safe savepoint name."""
        import re
    ...
    def _build_where_clause(self, where):
        ...
        for field, value in where.items():
            # Handle OR conditions (case-insensitive; preserves the legacy bare
            # "OR" spelling that the original ``field.upper() in (...)`` accepted)
    ...
    def _build_operator_condition(self, field, value_dict):
        ...
        for op, val in value_dict.items():
            ...
            if n_op == OP_LIKE:
                ...
                params.append(val)
            elif n_op == OP_IN and isinstance(val, list) and val:
                placeholders = ", ".join([self._get_single_placeholder() for _ in val])
                conditions.append(f"{field} IN ({placeholders})")
                params.extend(val)
            elif n_op == OP_NIN and isinstance(val, list) and val:
                placeholders = ", ".join([self._get_single_placeholder() for _ in val])
                conditions.append(f"{field} NOT IN ({placeholders})")
                params.extend(val)
            elif n_op == OP_NULL or (op_upper == "IS" and val is None):
                conditions.append(f"{field} IS NULL")
            elif n_op == OP_NOTNULL or (op_upper == "IS NOT" and val is None):
                conditions.append(f"{field} IS NOT NULL")
            else:
                sql_op = self._map_operator(n_op)
    ...
    def _map_operator(self, op):
        """Map a (canonical or raw) operator token to its SQL glyph.

        Normalizes first so any casing/alias ($GT, gt, >) resolves correctly —
        previously the lowercase-only map silently degraded $GT/$LT etc. to "=".
        """
        operator_map = {
            OP_EQ: _OP_EQ,
            OP_NE: "!=",
        ...
        return operator_map.get(normalize_operator(op), _OP_EQ)
    ...
    def _build_join_clause(self, tables, joins=None, schema=None, auto_join=False):
        ...
        if auto_enabled and schema:
            # Auto-detect joins from foreign key relationships (SSOT: join_policy)
            self._log('info', "[JOIN] Auto-joining tables (FK, %s) based on relationships", auto_join_type)
            from_clause, joined_tables = self._build_auto_join(tables, schema, base_table, auto_join_type)
        elif joins:
    ...
    def _build_auto_join(self, tables, schema, base_table, join_type=DEFAULT_AUTO_JOIN):
        """Auto-detect and build JOIN clauses from FK relationships.

        ``join_type`` is the canonical type resolved by join_policy (LEFT by
        default) — applied to every auto-detected FK link, forward and reverse.
        """
        from_clause = base_table
        joined_tables = []
        ...
        for table in remaining_tables:
            ...
            join_clause = self._try_forward_join_sql(table, base_table, joined_tables, schema, join_type)

            # Try reverse join (joined table has FK to this table)
            if not join_clause:
                join_clause = self._try_reverse_join_sql(table, base_table, joined_tables, schema, join_type)
    ...
    def _try_forward_join_sql(self, table, base_table, joined_tables, schema, join_type=DEFAULT_AUTO_JOIN):
        """Try to join table that has FK to already-joined tables."""
        keyword = to_sql_keyword(join_type)
        ...
        for field_name, field_def in table_schema.items():
            ...
            if ref_table == base_table or ref_table in joined_tables:
                ...
                self._log('debug', "  Auto-detected: %s %s ON %s", keyword, table, on_clause)
                return f" {keyword} {table} ON {on_clause}"

        return None

    def _try_reverse_join_sql(self, table, base_table, joined_tables, schema, join_type=DEFAULT_AUTO_JOIN):
        ...
        for already_joined in [base_table] + joined_tables:
            ...
            for field_name, field_def in joined_schema.items():
                ...
                if ref_table == table:
                    ...
                    self._log('debug', "  Auto-detected (reverse): %s %s ON %s", keyword, table, on_clause)
                    return f" {keyword} {table} ON {on_clause}"

        return None

    def _map_field_type(self, raw_type):
        """Map a field type string to the backend SQL type.

        Delegates to ``map_type`` so the dialect-correct table is always used —
        subclasses (SQLite, PostgreSQL) override ``map_type`` and this method
        inherits their resolution instead of re-implementing a divergent one.
        """
        return self.map_type(raw_type)

    def _index_spec_to_ddl(self, table_name, idx_spec):
        """
        Resolve one index spec (SSOT for naming) → (idx_name, create_sql).

        Accepts the same two forms as the schema `indexes:` key:
          * a bare field string → ``idx_<table>_<field>``
          * a dict ``{fields, unique, name}`` → composite / unique / custom-named
        Returns ``(None, None)`` for an unrecognized spec. Used by both the
        build-at-create path and the standalone ``create_index``, so an index
        gets the identical name however it's born (and can be dropped by field).
        """
        from ..index_naming import resolve_index_name, resolve_index_fields
        idx_name = resolve_index_name(table_name, idx_spec)
        fields = resolve_index_fields(idx_spec)
        if not idx_name or not fields:
            return None, None
        unique = "UNIQUE " if isinstance(idx_spec, dict) and idx_spec.get("unique") else ""
        return idx_name, (
            f"CREATE {unique}INDEX IF NOT EXISTS {idx_name} "
            f"ON {table_name} ({', '.join(fields)})"
        )

    def _create_indexes(self, table_name, indexes):
        """Create indexes for table (build-at-create path)."""
        cur = self.get_cursor()
        for idx_spec in indexes:
            _idx_name, sql = self._index_spec_to_ddl(table_name, idx_spec)
            if not sql:
    ...
    def create_index(self, table_name, index):
        """
        Create ONE index on an existing table; returns its name (or None if the
        spec is unrecognized). `IF NOT EXISTS` makes it idempotent. Honors an open
        transaction via ``_maybe_commit``.
        """
        idx_name, sql = self._index_spec_to_ddl(table_name, index)
        if not sql:
            self._log('warning', "Invalid index specification: %s", index)
            return None
        cur = self.get_cursor()
        self._log('info', "Creating index: %s", sql)
        cur.execute(sql)
    ...
    def drop_index(self, table_name, index):
        """
        Drop ONE index. `index` may be an explicit index name, a field string, a
        field list, or a dict (`{name}` / `{fields}`) — resolved to the SAME name
        `create_index` would have used, so add/drop are symmetric. Returns the
        dropped index name. `IF EXISTS` makes it a safe no-op when absent.
        """
        if isinstance(index, str):
            # explicit index name wins; otherwise resolve the field via the SSOT namer
            from ..index_naming import resolve_index_name
            idx_name = index if index.startswith("idx_") else resolve_index_name(table_name, index)
        else:
            idx_name, _ = self._index_spec_to_ddl(table_name, index)
        if not idx_name:
            self._log('warning', "Cannot resolve index to drop: %s", index)
            return None
        cur = self.get_cursor()
        sql = f"DROP INDEX IF EXISTS {idx_name}"
        self._log('info', "Dropping index: %s", sql)
        cur.execute(sql)
        self._maybe_commit()
        return idx_name

    def _constraint_body_sql(self, constraint):
        """Render the constraint body for ADD CONSTRAINT (fk / check). None if unknown."""
        ...
        if ctype in ("fk", "foreign_key"):
            ...
            if not (fields and ref_table and ref_col):
                return None
            body = (f"FOREIGN KEY ({', '.join(fields)}) "
                    f"REFERENCES {ref_table} ({ref_col})")
            on_delete = constraint.get("on_delete")
            if on_delete:
                body += f" ON DELETE {str(on_delete).upper()}"
            return body
        if ctype == "check":
    ...
    def add_constraint(self, table_name, constraint):
        """
        Add a fk/check constraint to an existing table (native SQL; Postgres-ready).

        Unique constraints never reach here — they ride the index pipeline. A
        duplicate-name ADD is tolerated (logged) so re-runs don't hard-fail.
        """
        name = constraint.get("name")
        body = self._constraint_body_sql(constraint)
        if not name or not body:
            self._log('warning', "Skipping unrenderable constraint: %s", constraint)
            return None
        sql = f"ALTER TABLE {table_name} ADD CONSTRAINT {name} {body}"
        try:
            cur = self.get_cursor()
            self._log('info', "Adding constraint: %s", sql)
            cur.execute(sql)
            self._maybe_commit()
        except Exception as exc:  # pylint: disable=broad-except
            self._log('warning', "add_constraint %s skipped: %s", name, exc)
            return None
        return name

    def drop_constraint(self, table_name, constraint):
        """Drop a named constraint (ALTER TABLE … DROP CONSTRAINT IF EXISTS)."""
        name = constraint.get("name") if isinstance(constraint, dict) else constraint
        if not name:
            self._log('warning', "Cannot resolve constraint to drop: %s", constraint)
            return None
        sql = f"ALTER TABLE {table_name} DROP CONSTRAINT IF EXISTS {name}"
        try:
            cur = self.get_cursor()
            self._log('info', "Dropping constraint: %s", sql)
            cur.execute(sql)
            self._maybe_commit()
        except Exception as exc:  # pylint: disable=broad-except
            self._log('warning', "drop_constraint %s skipped: %s", name, exc)
            return None
        return name
