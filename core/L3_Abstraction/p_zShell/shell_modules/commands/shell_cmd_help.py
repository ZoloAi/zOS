# zOS/core/L3_Abstraction/p_zShell/shell_modules/commands/shell_cmd_help.py
"""
...
Integration:
    # In shell executor
    execute_help(zos, {"args": []})          # Launch help UI
    execute_help(zos, {"args": ["ls"]})      # Show ls help directly

═══════════════════════════════════════════════════════════════════════════════
EXAMPLES
═══════════════════════════════════════════════════════════════════════════════

zCLI Mode (Direct Help):
    zOS> help ls
...
from zOS.L1_Foundation.a_zConfig.zConfig_modules.config_constants import SESSION_KEY_ZSPARK
...
def execute_help(zos: Any, parsed: Dict[str, Any]) -> None:
    """
    ...
    Args:
        zos: zOS framework instance with display, session, and walker subsystems
    ...
    Examples:
        >>> execute_help(zos, {"args": []})        # Launch help menu
        >>> execute_help(zos, {"args": ["ls"]})    # Show ls help directly
        >>> execute_help(zos, {"args": ["pwd"]})   # Show pwd help (handles aliases)
    ...
    if args:
        ...
        zos.logger.debug(LOG_SHOWING_COMMAND_HELP, command_name)
        _show_command_help(zos, command_name)
        return
    
    # Launch declarative help UI via walker
    zos.logger.debug(LOG_LAUNCHING_HELP)
    
    # Validate walker subsystem exists
    if not hasattr(zos, 'walker') or zos.walker is None:
        zos.logger.warning(LOG_WALKER_LAUNCH_FAILED, "walker subsystem not found")
        zos.display.error(MSG_WALKER_NOT_AVAILABLE)
        zos.display.info(MSG_USE_HELP_LIST)
        return
    
    try:
        # Set zSpark configuration for walker
        # Use zos.zspark_obj directly (always exists, not None)
        zos.zspark_obj["zVaFile"] = HELP_VAFILE_PATH
        zos.zspark_obj["zBlock"] = HELP_BLOCK_NAME
        
        # Launch walker (preserves current zMode: zCLI or zBifrost)
        zos.walker.run()
        
        zos.logger.debug(LOG_WALKER_LAUNCH_SUCCESS)
        
    except Exception as e:
        zos.logger.error(LOG_WALKER_LAUNCH_FAILED, str(e))
        zos.display.error(MSG_LAUNCH_FAILED.format(error=str(e)))
        zos.display.info(MSG_USE_HELP_LIST)


def _show_command_help(zos: Any, command_name: str) -> None:
    """
    ...
    Args:
        zos: zOS framework instance with display and logger
    ...
    Examples:
        >>> _show_command_help(zos, "ls")      # Direct command
        >>> _show_command_help(zos, "pwd")     # Alias → cwd
        >>> _show_command_help(zos, "list")    # Alias → ls
    ...
    if normalized_name not in SHELL_COMMANDS:
        zos.logger.warning(LOG_COMMAND_NOT_FOUND, command_name)
        zos.display.error(MSG_NO_HELP_AVAILABLE.format(command=command_name))
        zos.display.info(MSG_USE_HELP_LIST)
        return
    ...
    zos.display.header(header_text, style="box")
    ...
    zos.display.list(items, style="none")
