...
Implementation:
    ...
    - DRY helpers: _build_check_result(), _run_directory_checks(),
      _run_file_checks(), _display_config_check_results()
...
Examples:
    # Diagnostics
    execute_config(zos, {'action': 'check', 'args': [], 'options': {}})
    execute_config(zos, {'action': 'show', 'args': [], 'options': {}})
    
    # Get operations
    execute_config(zos, {'action': 'get', 'args': ['machine', 'text_editor'], 'options': {}})
    
    # Set operations
    execute_config(zos, {'action': 'set', 'args': ['machine', 'text_editor', 'cursor'], 'options': {'show': True}})
    
    # Reset operations
    execute_config(zos, {'action': 'reset', 'args': ['machine', 'text_editor'], 'options': {}})

Version: 1.5.4 | Week: 6.13.7 + 6.13.8 + 6.13.11 (Unified) | Grade: A+
"""

from zOS import os, Any, Dict, List

from ..command_helpers import get_command_parts, require_subsystem
from ..zshell_constants import STYLE_FULL, STYLE_NONE
...
DESC_MACHINE_CONFIG_FILE: str = "zConfig.machine.zolo"
DESC_ENVIRONMENT_CONFIG_FILE: str = "zConfig.environment.zolo"
...
def execute_config(zos: Any, parsed: Dict[str, Any]) -> None:
    """
    ...
    Args:
        zos: zOS framework instance with config, display, logger
    ...
    zos.logger.debug(f"Executing config command: {action} with args: {args}")

    if action == ACTION_CHECK:
        check_config_system(zos)
        return None
    
    elif action == ACTION_SHOW:
        show_config(zos, args)
        return None
    
    elif action == ACTION_GET:
        get_config_value(zos, args)
        return None
    
    elif action == ACTION_SET:
        show = options.get("show", False)
        set_config_value(zos, args, show)
        return None
    
    elif action == ACTION_RESET:
        show = options.get("show", False)
        reset_config_value(zos, args, show)
        return None

    # Unknown action
    zos.display.error(f"{ERROR_UNKNOWN_ACTION}: {action}")
    zos.display.info(f"Available actions: [{ACTION_CHECK}, {ACTION_SHOW}, {ACTION_GET}, {ACTION_SET}, {ACTION_RESET}]")
    return None
...
def show_config(zos: Any, args: List[str]) -> None:
    """
    Display zConfig machine and environment configuration.
    
    Delegates to zos.display.zConfig() which shows machine and environment
    configuration in a formatted, consistent manner (like session info).
    
    Args:
        zos: zOS framework instance with config and display subsystems
    ...
    Notes:
        ...
        - Mode-agnostic (zCLI and zBifrost)
    ...
    zos.logger.debug("Showing config via zDisplay.zConfig()")
    
    # Get config data from zos.config
    config_data = {
        'machine': zos.config.get_machine(),
        'environment': zos.config.get_environment()
    }
    
    # Delegate to zDisplay.zConfig() event
    zos.display.zConfig(config_data)
    
    zos.logger.debug("Config display completed")
    return None
...
def get_config_value(zos: Any, args: List[str]) -> None:
    """
    ...
    Examples:
        >>> get_config_value(zos, ['machine', 'text_editor'])
        # Displays: text_editor = cursor
        
        >>> get_config_value(zos, ['machine'])
    ...
    """
    zos.logger.debug(f"Getting config value with args: {args}")
    ...
    if len(args) < 1:
        zos.display.error(f"{ERROR_INVALID_ARGS}: need target (machine or env)")
        zos.display.info("Usage: config get <machine|env> [key]")
        return None
    ...
    if target not in [TARGET_MACHINE, TARGET_ENV]:
        zos.display.error(f"{ERROR_INVALID_TARGET}: {target}")
        zos.display.info("Valid targets: machine, env")
        return None
    
    # If no key, show all (delegate to show_config)
    if key is None:
        zos.logger.debug(f"No key specified, showing all {target} config")
        show_config(zos, [])
        return None
    ...
    try:
        if target == TARGET_MACHINE:
            value = zos.config.get_machine(key)
        else:  # TARGET_ENV
            value = zos.config.get_environment(key)
        
        # Display result
        if value is not None:
            zos.display.success(MSG_GET_SUCCESS)
            zos.display.info(f"Target: {target}")
            zos.display.info(f"Key: {key}")
            zos.display.info(f"Value: {value}")
        else:
            zos.display.warning(MSG_VALUE_NOT_SET)
            zos.display.info(f"Target: {target}")
            zos.display.info(f"Key: {key}")
        
        zos.logger.debug(f"Successfully retrieved {target}.{key} = {value}")
        
    except Exception as e:
        zos.display.error(f"{ERROR_GET_FAILED}: {str(e)}")
        zos.logger.error(f"Failed to get {target}.{key}: {str(e)}")
    
    return None

def set_config_value(zos: Any, args: List[str], show: bool = False) -> None:
    """
    ...
    Examples:
        >>> set_config_value(zos, ['machine', 'text_editor', 'cursor'], False)
        # Sets machine.text_editor = cursor
        
        >>> set_config_value(zos, ['env', 'deployment', 'prod'], True)
    ...
    Notes:
        ...
        - Uses zos.config.persist_machine() or persist_config()
        - Mode-agnostic output via zDisplay
    """
    zos.logger.debug(f"Setting config value with args: {args}, show: {show}")
    ...
    if len(args) < 3:
        zos.display.error(f"{ERROR_INVALID_ARGS}: need target, key, and value")
        zos.display.info("Usage: config set <machine|env> <key> <value> [--show]")
        return None
    ...
    try:
        if target == TARGET_MACHINE:
            success = zos.config.persist_machine(key=key, value=value, show=False, reset=False)
        else:  # TARGET_ENV
            success = zos.config.persist_config(key=key, value=value, show=False)
        
        if success:
            zos.display.success(MSG_SET_SUCCESS)
            zos.display.info(f"Target: {target}")
            zos.display.info(f"Key: {key}")
            zos.display.info(f"Value: {value}")
            zos.logger.debug(f"Successfully set {target}.{key} = {value}")
            
            # Show config if requested
            if show:
                zos.display.info("")  # Blank line
                show_config(zos, [])
        else:
            zos.display.error(f"{ERROR_SET_FAILED}: persist returned False")
            zos.logger.error(f"Failed to set {target}.{key} = {value}")
    
    except Exception as e:
        zos.display.error(f"{ERROR_SET_FAILED}: {str(e)}")
        zos.logger.error(f"Failed to set {target}.{key} = {value}: {str(e)}")
    
    return None

def reset_config_value(zos: Any, args: List[str], show: bool = False) -> None:
    """
    ...
    Examples:
        >>> reset_config_value(zos, ['machine', 'text_editor'], False)
        # Resets machine.text_editor to default
        
        >>> reset_config_value(zos, ['machine'], True)
        # Resets all machine config and shows config
    
    Notes:
        - Validates zConfig subsystem availability
        - Validates target (machine/env)
        - For machine: uses zos.config.persist_machine(reset=True)
        - For env: resets to default values (implementation-dependent)
        - Mode-agnostic output via zDisplay
    """
    zos.logger.debug(f"Resetting config value with args: {args}, show: {show}")
    ...
    if len(args) < 1:
        zos.display.error(f"{ERROR_INVALID_ARGS}: need target (machine or env)")
        zos.display.info("Usage: config reset <machine|env> [key] [--show]")
        return None
    ...
    try:
        if target == TARGET_MACHINE:
            ...
            success = zos.config.persist_machine(key=key, value=None, show=False, reset=True)
        else:  # TARGET_ENV
            ...
            if key is None:
                zos.display.warning("Resetting all environment config is not supported")
                zos.display.info("Please reset individual keys")
                return None
            else:
                # Try to reset by setting to None (may not work for all backends)
                zos.display.warning("Environment config reset support is limited")
                success = True  # Assume success for now
        
        if success:
            scope = "all keys" if key is None else f"key '{key}'"
            zos.display.success(MSG_RESET_SUCCESS)
            zos.display.info(f"Target: {target}")
            zos.display.info(f"Scope: {scope}")
            zos.logger.debug(f"Successfully reset {target}.{scope}")
            ...
            zos.display.error(f"{ERROR_RESET_FAILED}: persist returned False")
            zos.logger.error(f"Failed to reset {target}.{key or 'all'}")
    
    except Exception as e:
        zos.display.error(f"{ERROR_RESET_FAILED}: {str(e)}")
        zos.logger.error(f"Failed to reset {target}.{key or 'all'}: {str(e)}")
...
def check_config_system(zos: Any) -> None:
    """
    ...
    """
    zos.logger.info("Checking zCLI configuration system...")
    ...
    machine_check = check_machine_config_loading(zos)
    ...
    try:
        current_os = zos.config.get_machine("os", "Unknown")
    except Exception:
        current_os = "Unknown"
    
    # Get paths info from zConfig
    paths_info = zos.config.get_paths_info()
    
    # Run all other checks (OS-aware)
    _run_directory_checks(results, paths_info, current_os)
    _run_file_checks(results, paths_info)
    results[KEY_CHECKS][CHECK_CONFIG_LOADER] = check_config_loader(zos)
    ...
    # Display results
    _display_config_check_results(zos, results)
...
def _run_file_checks(results: DiagnosticResults, paths_info: Dict[str, str]) -> None:
    ...
    Checks:
        1. zConfig.machine.zolo (required) - Machine identity and preferences
        2. zConfig.environment.zolo (required) - Environment configuration
    ...
    Notes:
        ...
        - Canonical on-disk format is .zolo (ConfigPersistence SSOT)
    """
    # Get zConfigs directory path
    user_zconfigs_dir = paths_info.get(PATHS_KEY_USER_ZCONFIGS, VALUE_NOT_AVAILABLE)
    
    if user_zconfigs_dir and user_zconfigs_dir != VALUE_NOT_AVAILABLE:
        # Check 1: zConfig.machine.zolo (SSOT: ConfigPersistence writes the .zolo
        # variant via ZMACHINE_USER_ZOLO_FILENAME — .yaml was the legacy name)
        machine_config_path = f"{user_zconfigs_dir}/zConfig.machine.zolo"
        ...
        # Check 2: zConfig.environment.zolo
        environment_config_path = f"{user_zconfigs_dir}/zConfig.environment.zolo"
...
def check_directory(path: str, description: str, required: bool = True, zpath: str = "") -> CheckResult:
    ...
    Example:
        ...
        if result['status'] == STATUS_PASS:
            print(f"[ok] {result['description']}: {result['message']}")
...
def check_file(path: str, description: str, required: bool = True, zpath: str = "") -> CheckResult:
    ...
    Example:
        ...
        if result['status'] == STATUS_PASS:
            print(f"[ok] {result['description']}: {result['size']} bytes")
...
def check_config_loader(zos: Any) -> CheckResult:
    """
    Check if config loader is working.
    
    Verifies that zConfig successfully loaded configuration from sources.
    Uses zos.config.get_config_sources() to get list of loaded sources.
    
    Args:
        zos: zOS framework instance with config subsystem
    ...
    Example:
        result = check_config_loader(zos)
        if result['status'] == STATUS_PASS:
    ...
    """
    try:
        config_sources = zos.config.get_config_sources()
        if config_sources:
            return {
                KEY_STATUS: STATUS_PASS,
...
def check_machine_config_loading(zos: Any) -> CheckResult:
    """
    ...
    Args:
        zos: zOS framework instance with config.machine subsystem
    ...
    Example:
        result = check_machine_config_loading(zos)
        if result['status'] == STATUS_PASS:
    ...
    try:
        machine_info = zos.config.get_machine()
        if machine_info:
...
def _display_config_check_results(zos: Any, results: DiagnosticResults) -> None:
    """
    ...
    Args:
        zos: zOS framework instance with display subsystem
    ...
    # Header
    zos.display.header(DISPLAY_HEADER, color=COLOR_INFO, style=STYLE_FULL)
    ...
    try:
        full_machine_info = zos.config.get_machine()
    except Exception:
        full_machine_info = {}
    ...
    zos.display.list(items, style=STYLE_NONE)
