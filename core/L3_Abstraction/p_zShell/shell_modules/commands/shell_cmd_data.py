# zOS/core/L3_Abstraction/p_zShell/shell_modules/commands/shell_cmd_data.py
"""
...
SELECT and HEAD operations use zDisplay's AdvancedData event package:
    ...
    - zCLI mode: ASCII tables with "Press Enter to continue..."
...
from zOS import TYPE_CHECKING, Dict, List, Any, Optional

from ..executor_constants import (
    KEY_ACTION as EC_KEY_ACTION,
    KEY_ARGS as EC_KEY_ARGS,
    KEY_OPTIONS as EC_KEY_OPTIONS,
)

if TYPE_CHECKING:
    from zOS.zCLI import zCLI
...
# --- CRUD Actions (5) ---
# NOTE: action vocabulary is owned by zParser (VALID_DATA_ACTIONS); the parser
# emits "read" (not "select"), so the shell must validate/route on "read".
...
CRUD_ACTIONS = {ACTION_INSERT, ACTION_READ, ACTION_UPDATE, ACTION_DELETE, ACTION_UPSERT}
...
# --- Parsed Dict Keys --- SSOT: executor_constants
PARSED_ACTION = EC_KEY_ACTION
PARSED_ARGS = EC_KEY_ARGS
PARSED_OPTIONS = EC_KEY_OPTIONS
...
def _validate_zdata_subsystem(zos: 'zCLI') -> Optional[str]:
    """
    ...
    Example:
        >>> error = _validate_zdata_subsystem(zos)
        >>> if error:
        >>>     zos.display.error(error)
        >>>     return
    """
...
def _build_request(
    ...
    Example:
        ...
        >>> zos.data.handle_request(request)
    """
...
def _handle_migration_action(
    ...
    Example:
        >>> _handle_migration_action(zos, "migrate", {"model": "@.Schema.v2", "dry_run": True})
    """
    try:
        if action == ACTION_MIGRATE:
            ...
            if not new_schema_path:
                zos.display.error(ERROR_MISSING_SCHEMA)
                return
            ...
            if result and not dry_run:
                zos.display.success(SUCCESS_MIGRATED.format(summary="schema updated"))
            ...
            zos.logger.debug("Retrieving migration history: limit=%s", limit)
            
            history = zos.data.get_migration_history(limit=limit)
            ...
            if history:
                ...
                zos.display.info("No migration history found")
    
    except Exception as e:
        zos.logger.error("Migration action failed: %s", str(e))
        zos.display.error(ERROR_MIGRATION_FAILED.format(error=str(e)))
...
def _handle_connection_action(
    ...
    Example:
        >>> _handle_connection_action(zos, "connect", {"backend": "sqlite", "model": "@.Schema.myapp"})
    """
    try:
        if action == ACTION_CONNECT:
            ...
            if backend not in SUPPORTED_BACKENDS:
                zos.display.error(ERROR_INVALID_BACKEND.format(backend=backend))
                return
            
            # Validate model
            if not model:
                zos.display.error(ERROR_MISSING_MODEL)
                return
            
            # Call zData.connect()
            zos.logger.debug("Connecting to backend: %s, model: %s", backend, model)
            zos.data.connect(backend=backend, schema_path=model)
            zos.display.success(SUCCESS_CONNECTED.format(backend=backend))
        
        elif action == ACTION_DISCONNECT:
            # Call zData.disconnect()
            zos.logger.debug("Disconnecting from backend")
            zos.data.disconnect()
            zos.display.success(SUCCESS_DISCONNECTED)
        
        elif action == ACTION_STATUS:
            # Check connection status
            is_connected = zos.data.is_connected()
            
            if is_connected:
                # Get backend info (if available)
                backend_type = getattr(zos.data.adapter, '__class__.__name__', 'Unknown')
                zos.display.success(SUCCESS_STATUS_CONNECTED.format(backend=backend_type))
            else:
                zos.display.info(SUCCESS_STATUS_DISCONNECTED)
    
    except Exception as e:
        zos.logger.error("Connection action failed: %s", str(e))
        
        if action == ACTION_CONNECT:
            zos.display.error(ERROR_CONNECTION_FAILED.format(error=str(e)))
        elif action == ACTION_DISCONNECT:
            zos.display.error(ERROR_DISCONNECT_FAILED.format(error=str(e)))
        else:
            zos.display.error(ERROR_STATUS_CHECK_FAILED.format(error=str(e)))
...
def execute_data(zos: 'zCLI', parsed: Dict[str, Any]) -> None:
    """
    ...
    This is the main entry point called by zShell's command router. It:
        ...
        3. Routes to appropriate handler:
           ...
           - CRUD/DDL actions → zos.data.handle_request()
        4. Handles all errors gracefully with user-friendly messages
    
    Args:
        zos: The zCLI instance with access to all subsystems
    ...
    try:
        ...
        if not action:
            zos.display.error(ERROR_MISSING_ARGS.format(action="<unknown>"))
            return
        ...
        if action in MIGRATION_ACTIONS:
            _handle_migration_action(zos, action, options)
            return
        
        # Route connection actions to special handler
        if action in CONNECTION_ACTIONS:
            _handle_connection_action(zos, action, options)
            return
        ...
        zos.data.handle_request(zRequest)
    
    except KeyError as e:
        # Missing required key in parsed dict
        zos.logger.error("Missing required key in parsed command: %s", str(e))
        zos.display.error(ERROR_INVALID_ARGS.format(error=f"Missing key: {e}"))
    
    except ValueError as e:
        # Invalid value (e.g., type conversion failed)
        zos.logger.error("Invalid value in data command: %s", str(e))
        zos.display.error(ERROR_INVALID_ARGS.format(error=str(e)))
    
    except Exception as e:
        # Catch-all for unexpected errors
        zos.logger.error("Data operation failed with unexpected error: %s", str(e))
        zos.display.error(ERROR_EXECUTION_FAILED.format(error=str(e)))
