# zOS/core/L3_Abstraction/p_zShell/shell_modules/commands/shell_cmd_ls.py
...
from zOS import os, Path, Any, Dict, List, Optional

# zPath resolution SSOT (shared by cd, ls, where, session, runner)
from ..shell_paths import resolve_nav_path
...
def execute_ls(zos: Any, parsed: Dict[str, Any]) -> None:
    """
    ...
    Args:
        zos: The zCLI instance providing access to session, display, and parser
    ...
    Examples:
        >>> execute_ls(zos, {"args": [], "options": {}})
        # Lists current working directory (works for list/ls/dir)
        
        >>> execute_ls(zos, {"args": ["@.src"], "options": {"size": True}})
        # Lists workspace/src with file sizes (modern: -size)
        
        >>> execute_ls(zos, {"args": ["@.src"], "options": {"long": True}})
        # Lists workspace/src with file sizes (Unix: -l)
        
        >>> execute_ls(zos, {"args": ["~.Projects"], "options": {"hidden": True}})
        # Lists ~/Projects including hidden files (modern: -hidden)
        
        >>> execute_ls(zos, {"args": ["@.src"], "options": {"file": True}})
    ...
    resolved: Optional[Path] = _resolve_zpath(zos, target)
    if resolved is None:
        return None  # Error already displayed
    
    # Validate path exists and is a directory
    if not resolved.exists():
        zos.display.error(MSG_PATH_NOT_FOUND.format(path=resolved))
        return None
    
    if not resolved.is_dir():
        zos.display.error(MSG_NOT_A_DIRECTORY.format(path=resolved))
        return None
    ...
    try:
        ...
        _display_entries_bulk(zos, resolved, entries, options)
        
        return None  # Success - output already displayed
        
    except PermissionError:
        zos.display.error(MSG_PERMISSION_DENIED.format(path=resolved))
        return None
    except (OSError, IOError) as e:
        zos.display.error(MSG_FILESYSTEM_ERROR.format(error=str(e)))
        return None
...
def _resolve_zpath(zos: Any, target: str) -> Optional[Path]:
    """
    ...
    Args:
        zos: The zCLI instance for session access
    ...
    Examples:
        >>> _resolve_zpath(zos, "@.src")
        Path('/workspace/src')
        
        >>> _resolve_zpath(zos, "~.Projects")
    ...
    """
    try:
        # Delegate to the shell-wide zPath SSOT (shell_paths.resolve_nav_path):
        # handles @., ~zMachine, ~., ~, .., ., absolute and relative-to-cwd.
        return resolve_nav_path(zos, target)
...
def _display_entries_bulk(
    ...
    Args:
        zos: The zCLI instance for display access
    ...
    if not entries:
        ...
        zos.display.list(items, style=DEFAULT_LIST_STYLE)
        return
