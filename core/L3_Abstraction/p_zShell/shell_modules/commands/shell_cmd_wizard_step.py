# zOS/core/L3_Abstraction/p_zShell/shell_modules/commands/shell_cmd_wizard_step.py

"""
Shell-specific step execution.

Handles shell-mode command parsing and routing:
...
from zOS import Dict, Any, Optional, Union
...
def execute_wizard_step(
    ...
    Args:
        zos: The zCLI instance providing access to all subsystems
    ...
    Step Types Handled:
        ...
        2. **zFunc Operations** (Dict with "zFunc" key or str starting with "zFunc("):
           - Delegates to zos.zfunc.handle() for function resolution
        ...
        4. **Shell Commands** (str without "zFunc(" prefix):
           - Parses command using zos.zparser.parse_command()
        ...
        5. **Generic Dict** (Dict without recognized keys):
           ...
           - Passed to zos.data.handle_request()
    ...
    Examples:
        ...
        ...     }
        ... }
        >>> result = execute_wizard_step(zos, "get_users", step_value, logger, context)
        
        >>> # Example 2: zFunc expression
        >>> step_value = "zFunc(hash_password, 'mypass')"
        >>> result = execute_wizard_step(zos, "hash_pw", step_value, logger)
        ...
        ...     }
        ... }
        >>> execute_wizard_step(zos, "notify", step_value, logger)
        
        >>> # Example 4: Shell command
        >>> step_value = "data read users where active=true"
        >>> result = execute_wizard_step(zos, "query", step_value, logger)
    ...
    if isinstance(step_value, dict):
        # Step-level zRBAC: check before executing any step type
        if 'zRBAC' in step_value:
            zrbac = step_value.get('zRBAC')
            if isinstance(zrbac, dict):
                check_fn = getattr(getattr(zos, 'dispatch', None), '_check_zrbac', None)
                if check_fn is not None and not check_fn(zrbac):
                    return None  # RBAC blocked — step skipped; zHat records None
            step_value = {k: v for k, v in step_value.items() if k != 'zRBAC'}
                ...
                if model and isinstance(model, str) and model.startswith("$"):
                    ...
                    if context and context.get(CACHE_KEY_SCHEMA):
                        ...
                        if schema_cache.has_connection(alias_name):
                            ...
                            try:
                                return zos.data.handle_request(zdata_config, context=context)
                            except Exception as e:  # pylint: disable=broad-except
                                logger.error("%s: %s", ERROR_MSG_DATA_REQUEST, e)
                                return None
                    
                    # Check pinned_cache for schema data
                    cached_schema = zos.loader.cache.pinned_cache.get_alias(alias_name)
                    if cached_schema:
                        ...
                        # Alias not in pinned_cache or schema_cache.
                        # Still wire _alias_name so _init_wizard_handler is triggered,
                        # which will fall back to SchemaManager._server_registry
                        # (populated at boot from all registered zSchema files).
                        if CONFIG_KEY_OPTIONS not in zdata_config:
                            zdata_config[CONFIG_KEY_OPTIONS] = {}
                        zdata_config[CONFIG_KEY_OPTIONS][OPTION_ALIAS_NAME] = alias_name
                        zdata_config[CONFIG_KEY_MODEL] = None  # Clear model, let wizard handler resolve
                        logger.debug("Alias $%s not pinned — deferring to server registry bootstrap", alias_name)
        ...
        if 'zInput' in step_value:
            # zInput shorthand — expand to read_string display event
            zinput_cfg = dict(step_value['zInput']) if isinstance(step_value['zInput'], dict) else {}
            zinput_cfg['event'] = 'read_string'
            zinput_cfg.setdefault('type', 'text')
            zinput_cfg.setdefault('default', '')
            zinput_cfg.setdefault('placeholder', '')
            zinput_cfg.setdefault('required', False)
            return zos.display.handle(zinput_cfg)

        if STEP_KEY_ZFUNC in step_value:
            # zFunc operation
            func_expr = step_value[STEP_KEY_ZFUNC]
            try:
                return zos.zfunc.handle(func_expr, zContext=context)
            except Exception as e:  # pylint: disable=broad-except
        ...
        if STEP_KEY_ZDISPLAY in step_value:
            ...
            if isinstance(display_obj, dict):
                ...
                if event in [EVENT_TEXT, EVENT_LINE]:
                    ...
                    return zos.display.text(content)
                if event == EVENT_HEADER:
                    content = display_obj.get(CONFIG_KEY_CONTENT, "")
                    return zos.display.header(content)
                if event == EVENT_BREAK:
                    return zos.display.break_line()
                if event in [EVENT_ERROR, EVENT_WARNING, EVENT_SUCCESS, EVENT_INFO]:
                    message = display_obj.get(CONFIG_KEY_MESSAGE, "")
                    return getattr(zos.display, event)(message)
                if event == EVENT_JSON:
                    data = display_obj.get(CONFIG_KEY_DATA, {})
                    return zos.display.json(data)
                
                # Fallback: route through display.handle for any unhandled events
                return zos.display.handle(display_obj)
            
            logger.error(ERROR_MSG_ZDISPLAY_DICT)
            return None
        
        if 'zDialog' in step_value:
            try:
                from zOS.L2_Handling.j_zDialog import handle_zDialog  # pylint: disable=import-outside-toplevel
                dialog_spec = {'zDialog': step_value['zDialog']}
                result = handle_zDialog(dialog_spec, zcli=zos, context=context)
                # Return truthy sentinel so wizard's zHat records this step as run.
                # handle_zDialog returns None when onSubmit dispatches zVar/zLink etc.
                return result if result is not None else True
            except Exception as e:  # pylint: disable=broad-except
                logger.error("zDialog step failed: %s", e)
                return None

        if 'zVar' in step_value:
            try:
                _walker = getattr(zos, 'walker', None)
                return zos.dispatch.handle('zVar', {'zVar': step_value['zVar']}, context=context, walker=_walker)
            except Exception as e:  # pylint: disable=broad-except
                logger.error("zVar step failed: %s", e)
                return None

        if 'zDelta' in step_value:
            try:
                _walker = getattr(zos, 'walker', None)
                return zos.dispatch.handle('zDelta', {'zDelta': step_value['zDelta']}, context=context, walker=_walker)
            except Exception as e:  # pylint: disable=broad-except
                logger.error("zDelta step failed: %s", e)
                return None

        # Generic dict - try as zData request
        try:
            return zos.data.handle_request(step_value, context=context)
        except Exception as e:  # pylint: disable=broad-except
            logger.error("%s: %s", ERROR_MSG_DATA_REQUEST, e)
            return None
    ...
    if isinstance(step_value, str):
        ...
        if step_value.startswith(ZFUNC_PREFIX):
            try:
                return zos.zfunc.handle(step_value)
            except Exception as e:  # pylint: disable=broad-except
                logger.error("%s: %s", ERROR_MSG_FUNC_HANDLE, e)
                return None
        ...
        if command_type == CMD_TYPE_DATA:
            try:
                ...
                return execute_data(zos, parsed)
            except Exception as e:  # pylint: disable=broad-except
        ...
        if command_type == CMD_TYPE_FUNC:
            try:
                ...
                return execute_func(zos, parsed)
            except Exception as e:  # pylint: disable=broad-except
        ...
        if command_type == CMD_TYPE_LOAD:
            try:
                ...
                return execute_load(zos, parsed)
            except Exception as e:  # pylint: disable=broad-except
