...
def execute_export(zos: Any, parsed: Dict[str, Any]) -> None:
    """
    ...
    Examples:
        # OLD (deprecated)
        execute_export(zos, {'action': 'machine', 'args': ['text_editor', 'cursor'], 'options': {}})
        
        # NEW (unified)
        execute_config(zos, {'action': 'set', 'args': ['machine', 'text_editor', 'cursor'], 'options': {}})
    ...
    zos.display.warning(DEPRECATION_WARNING)
    zos.logger.warning("export command is deprecated, use 'config set' or 'config reset'")
    ...
    if reset:
        ...
        reset_config_value(zos, reset_args, show)
    else:
        # Set operation
        if key is None or value is None:
            zos.display.error("Missing required arguments for set operation")
            zos.display.info("Usage: config set <machine|env> <key> <value> [--show]")
            return None
        
        set_args = [target, key, value]
        set_config_value(zos, set_args, show)
