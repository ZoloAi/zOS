# zOS/core/L3_Abstraction/p_zShell/shell_modules/commands/shell_cmd_walker.py
...
OVERVIEW:
    ...
    declarative UI blocks defined in zVaFiles, supporting both zCLI and zBifrost
...
COMMAND SYNTAX:
    ...
    Required session fields:
        ...
        - zBlock: Starting block name (e.g., "zVaF", "Help")
...
WALKER FLOW:
    ...
    2. Mode Preservation: Store current zMode (zCLI/zBifrost)
...
ZAUTH AWARENESS:
    Walker inherits zAuth context from session (SESSION_KEY_ZVISITOR). When navigating
...
MODE HANDLING:
    Walker preserves the session's zMode (zCLI or zBifrost) throughout execution.
...
ZSPARK CONFIGURATION:
    Walker updates zspark_obj (zSpark context) with session configuration:
        ...
        - zMode: zCLI/zBifrost (preserved from session)
...
FUTURE ENHANCEMENTS:
    ...
    • --mode flag to override session mode (force zCLI or zBifrost)
...
from ..executor_constants import KEY_ACTION
...
# Default Values
DEFAULT_ZMODE: str = "zCLI"
DEFAULT_ZVAFOLDER: str = "@"
DEFAULT_ZBLOCK: str = "zVaF"
...
LOG_CREATING_WALKER: str = "Creating zWalker instance from zOS..."
...
def execute_walker(zos: Any, parsed: Dict[str, Any]) -> Optional[Dict[str, str]]:
    """
    ...
    (zCLI/zBifrost) throughout execution.
    
    Args:
        zos: zOS framework instance with full subsystem access
            - zos.session: Session dictionary (reads 4 required fields)
            - zos.zspark_obj: zSpark context (updates with walker config)
            - zos.logger: Logger instance (logs walker lifecycle)
            - zos.display: zDisplay instance (future: user feedback)
    ...
    Session Requirements:
        ...
        4. zBlock: Starting block name for walker
           Set via: session set zBlock zVaF
    ...
    Mode Handling:
        Walker preserves session zMode (zCLI or zBifrost) throughout execution.
    ...
    zAuth Context:
        Walker inherits zAuth context from SESSION_KEY_ZVISITOR in session. No explicit
    ...
    Notes:
        ...
        • Walker respects session mode (zCLI displays in terminal, Bifrost via WebSocket)
    ...
    if action == ACTION_RUN:
        ...
        zos.logger.info(LOG_LAUNCHING_WALKER)
        
        # Validate required session fields (Phase 1)
        validation_error = _validate_walker_session(zos)
        if validation_error:
            return validation_error
        
        # Store current mode for defensive restoration (Phase 2)
        current_mode = zos.session.get(SESSION_KEY_ZMODE, DEFAULT_ZMODE)
        
        # Build zSpark configuration from session (Phase 3)
        _build_walker_zspark(zos)
        
        # Import and launch Walker (Phase 4)
        try:
            from zOS.L4_Orchestration.q_zWalker.zWalker import zWalker
            
            zos.logger.info(LOG_CREATING_WALKER)
            walker = zWalker(zos)
            
            zos.logger.info(LOG_STARTING_WALKER, current_mode)
            result = walker.run()
            
            # Walker exited normally - restore mode and return success
            zos.logger.info(LOG_WALKER_EXITED)
            _restore_session_mode(zos, current_mode)
            ...
            zos.logger.error(LOG_WALKER_ERROR, e, exc_info=True)
            _restore_session_mode(zos, current_mode)
            ...
            zos.logger.info(LOG_WALKER_EXIT_SYSEXIT, e.code)
            _restore_session_mode(zos, current_mode)
...
def _validate_walker_session(zos: Any) -> Optional[Dict[str, str]]:
    """
    ...
    Args:
        zos: zOS framework instance with session access
            - zos.session: Session dictionary to validate
    ...
    for field in required_fields:
        ...
        if not zos.session.get(session_key):
...
def _build_walker_zspark(zos: Any) -> None:
    """
    ...
    full zPath from zVaFolder + zVaFile, and updates zos.zspark_obj with walker
    initialization context. zSpark provides context to subsystems during walker
    execution.
    
    Args:
        zos: zOS framework instance with session and zspark_obj access
            - zos.session: Session dictionary (reads 5 fields)
            - zos.zspark_obj: zSpark context dict (updates 5 fields)
    
    Returns:
        None (updates zos.zspark_obj in place)
    ...
    zSpark Fields Updated:
        ...
        5. zMode: zCLI or zBifrost (from SESSION_KEY_ZMODE)
    ...
    Default Values:
        ...
        - zBlock: "zVaF" (default starting block)
        - zMode: "zCLI" (default mode)
    ...
    Examples:
        ...
        # After _build_walker_zspark(zos)
        zspark_obj = {
            "zSpace": "/Users/name/workspace",
    ...
    zva_folder = zos.session.get(SESSION_KEY_ZVAFOLDER, DEFAULT_ZVAFOLDER)
    zva_file = zos.session[SESSION_KEY_ZVAFILE]  # Required (validated)
    zblock = zos.session.get(SESSION_KEY_ZBLOCK, DEFAULT_ZBLOCK)
    current_mode = zos.session.get(SESSION_KEY_ZMODE, DEFAULT_ZMODE)
    ...
    zos.zspark_obj.update({
        SESSION_KEY_ZSPACE: zos.session[SESSION_KEY_ZSPACE],  # Workspace root
        ...
        SESSION_KEY_ZMODE: current_mode      # zCLI or zBifrost
    })
    ...
    if "browser" in zos.zspark_obj and zos.zspark_obj["browser"] is not None:
        zos.session[SESSION_KEY_BROWSER] = zos.zspark_obj["browser"]
        zos.logger.debug("Walker: session[browser] set from zSpark: %s", zos.zspark_obj["browser"])
    
    if "ide" in zos.zspark_obj and zos.zspark_obj["ide"] is not None:
        zos.session[SESSION_KEY_IDE] = zos.zspark_obj["ide"]
        zos.logger.debug("Walker: session[ide] set from zSpark: %s", zos.zspark_obj["ide"])


def _restore_session_mode(zos: Any, original_mode: str) -> None:
    """
    ...
    Args:
        zos: zOS framework instance with session access
            - zos.session: Session dictionary (updates zMode field)
            - zos.logger: Logger instance (logs mode restoration)
        
        original_mode: Mode captured before walker launch
            Expected values: "zCLI" or "zBifrost"
    
    Returns:
        None (updates zos.session[SESSION_KEY_ZMODE] in place)
    ...
    Mode Values:
        - "zCLI": CLI terminal mode (stdout display)
        - "zBifrost": WebSocket mode (browser display)
    
    Examples:
        # Before walker launch
        current_mode = zos.session.get("zMode", "zCLI")  # "zCLI"
        ...
        _restore_session_mode(zos, current_mode)
        # zos.session["zMode"] → "zCLI" (guaranteed)
    ...
    """
    zos.session[SESSION_KEY_ZMODE] = original_mode
    zos.logger.info(LOG_MODE_RESTORED, original_mode)
