# zOS/core/L3_Abstraction/p_zShell/shell_modules/commands/shell_cmd_func.py
...
ZFUNC INTEGRATION:
    ...
    Delegation flow:
        ...
        4. func command calls: zos.zfunc.handle(expression)
...
AUTO-INJECTION:
    ...
    Example function signatures:
        ...
        def zcli_aware_function(zos, arg1):
            zos.logger.info("Processing: %s", arg1)
            return arg1
        
        # Both zcli and session injected
        def session_aware_function(zos, session, arg1):
...
ZSESSION AWARENESS:
    ...
    Common session keys available:
        ...
        • SESSION_KEY_ZMODE: zCLI or zBifrost
        • SESSION_KEY_ZVISITOR: Auth context (if authenticated)
...
ZAUTH AWARENESS:
    Functions can access authentication context via session["zLobby"] when present.
    ...
    Example auth-aware function:
        def admin_only_function(zos, session):
            auth = session.get("zLobby", {})
...
ZPARSER INTEGRATION:
    zFunc uses zos.zparser for all path resolution and argument parsing:
...
# Dict Keys (for refactor-proof access) — SSOT: executor_constants (imported above)
...
def _validate_zfunc_subsystem(zos: Any) -> Optional[Dict[str, str]]:
    """
    Validate that zFunc subsystem is available in zcli instance.
    
    Defensive check to ensure zos.zfunc exists and is accessible before
    attempting to call it. This prevents AttributeError crashes if zFunc
    was not initialized properly.
    
    Args:
        zos: zOS framework instance to validate
    ...
    Example:
        >>> error = _validate_zfunc_subsystem(zos)
        >>> if error:
        ...     return error  # Abort execution
    """
    if not hasattr(zos, 'zfunc'):
        zos.logger.error(ERROR_NO_ZFUNC)
        return {
            KEY_ERROR: ERROR_NO_ZFUNC,
            KEY_DETAILS: "zFunc subsystem not found in zcli instance"
        }
    
    if not hasattr(zos.zfunc, 'handle'):
        zos.logger.error("zFunc subsystem exists but missing handle() method")
        return {
            KEY_ERROR: ERROR_NO_ZFUNC,
            KEY_DETAILS: "zFunc.handle() method not found"
        }
    
    zos.logger.debug("zFunc subsystem validation: OK")
    return None


def _validate_parsed_command(zos: Any, parsed: Dict[str, Any]) -> Optional[Dict[str, str]]:
    """
    ...
    Args:
        zos: zOS framework instance for logging
    ...
    try:
        ...
        if KEY_ACTION not in parsed:
            zos.logger.error(ERROR_MISSING_ACTION)
            return {
        ...
        if KEY_ARGS not in parsed:
            zos.logger.error("Parsed command missing 'args' key")
            return {
        ...
        if not isinstance(parsed[KEY_ARGS], list):
            zos.logger.error("'args' is not a list: %s", type(parsed[KEY_ARGS]))
            return {
        ...
        if not func_name or (isinstance(func_name, str) and not func_name.strip()):
            zos.logger.error(ERROR_EMPTY_FUNCTION_NAME)
            return {
                KEY_ERROR: ERROR_EMPTY_FUNCTION_NAME,
                KEY_DETAILS: "Function name cannot be empty string or whitespace"
            }
        
        zos.logger.debug("Parsed command validation: OK")
        return None
        
    except Exception as e:
        zos.logger.error("Parsed command validation failed: %s", e, exc_info=True)
        return {
...
def _validate_function_name(zos: Any, func_name: str) -> Optional[str]:
    """
    ...
    if not func_name or not func_name.strip():
        zos.logger.warning(ERROR_EMPTY_FUNCTION_NAME)
    ...
    if len(func_name) > MAX_FUNCTION_NAME_LENGTH:
        ...
        zos.logger.warning(error)
        return error
    
    zos.logger.debug("Function name validation: OK (%d chars)", len(func_name))
    return None


def _validate_args_count(zos: Any, args: List[str]) -> Optional[str]:
    """
    ...
    zos.logger.debug("Argument count validation: OK (%d args)", len(args))
    return None
...
def _format_args(zos: Any, args: List[str]) -> str:
    """
    ...
    Example:
        >>> _format_args(zos, ["data", "123"])
        ", data, 123"
        >>> _format_args(zos, [])
        ""
    """
    if not args:
        zos.logger.debug("No arguments to format")
        return ""
    ...
    if not valid_args:
        zos.logger.debug("All arguments were None/empty, returning empty string")
        return ""
    
    args_str = TEMPLATE_ARG_SEP + TEMPLATE_ARG_SEP.join(valid_args)
    zos.logger.debug("Formatted %d args: %s", len(valid_args), args_str)
    return args_str


def _build_func_expression(zos: Any, func_name: str, args: List[str]) -> str:
    """
    ...
    zos.zfunc.handle(). Uses template for consistent formatting.
    ...
    Example:
        >>> _build_func_expression(zos, "@.utils.helper", ["data"])
        "zFunc(@.utils.helper, data)"
    """
    args_str = _format_args(zos, args)
    func_expr = TEMPLATE_FUNC_EXPR.format(func_name=func_name, args_str=args_str)
    zos.logger.debug("Built zFunc expression: %s", func_expr)
    return func_expr
...
def _handle_execution_error(
    ...
    zos.logger.error("Expression: %s", func_expr)
    zos.logger.error("Error type: %s", error_type)
    zos.logger.error("Error message: %s", error_msg)
...
def execute_func(zos: Any, parsed: Dict[str, Any]) -> Any:
    """
    ...
    builds a zFunc expression, and delegates execution to zos.zfunc.handle().
    ...
    Args:
        zos: zOS framework instance with zfunc subsystem, logger, and session
    ...
    Execution flow:
        ...
        7. Delegate to zos.zfunc.handle()
    ...
    Examples:
        Basic function call:
            ...
            ...     "options": {}
            ... }
            >>> result = execute_func(zos, parsed)
            >>> # Returns: "1.5.4"
        ...
        Function with arguments:
            ...
            ... }
            >>> result = execute_func(zos, parsed)
            >>> # Calls: process(zos, session, "data", "123")
    ...
    error = _validate_parsed_command(zos, parsed)
    if error:
        return error
    ...
    zos.logger.debug("Executing function: %s", func_name)
    zos.logger.debug("Arguments: %s", args)
    
    # Step 4: Validate function name format
    error_msg = _validate_function_name(zos, func_name)
    if error_msg:
        return {
            KEY_ERROR: error_msg,
    ...
    error_msg = _validate_args_count(zos, args)
    if error_msg:
        return {
            KEY_ERROR: error_msg,
    ...
    func_expr = _build_func_expression(zos, func_name, args)
    zos.logger.info("Executing: %s", func_expr)
    
    # Step 7: Delegate to zFunc for execution
    try:
        result = zos.zfunc.handle(func_expr)
        zos.logger.debug("Function execution completed successfully")
        return result
        
    except AttributeError as e:
        # Function not found in module
        zos.logger.error("AttributeError during function execution", exc_info=True)
        return _handle_execution_error(zos, e, func_name, func_expr)
        
    except ImportError as e:
        # Module import failed
        zos.logger.error("ImportError during function execution", exc_info=True)
        return _handle_execution_error(zos, e, func_name, func_expr)
        
    except FileNotFoundError as e:
        # Python file not found
        zos.logger.error("FileNotFoundError during function execution", exc_info=True)
        return _handle_execution_error(zos, e, func_name, func_expr)
        
    except Exception as e:
        # Any other execution error
        zos.logger.error("Unexpected error during function execution", exc_info=True)
        return _handle_execution_error(zos, e, func_name, func_expr)
