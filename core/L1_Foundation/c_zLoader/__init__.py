# zOS/core/L1_Foundation/c_zLoader/__init__.py
...
providing the main entry point for importing zLoader into zOS framework. It exposes the zLoader
...
**Tier 6 - Package Root (Entry Point)**
    ...
    - Used By: zOS framework (imports zLoader for zos.loader attribute)
...
Usage Patterns
--------------
**Standard Import (in zOS framework)**:
    >>> from zOS.L1_Foundation.c_zLoader import zLoader
    >>> self.loader = zLoader(self)
    >>> # Used as: zos.loader.handle(zPath)
...
------------------
**Week 6.6 (zDispatch)**:
    - dispatch_launcher.py: self.zos.loader.handle(zVaFile)
    - dispatch_modifiers.py: self.zos.loader.handle(zVaFile)
