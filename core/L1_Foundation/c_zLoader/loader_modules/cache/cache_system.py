# zOS/core/L1_Foundation/c_zLoader/loader_modules/cache/cache_system.py
...
--------------
**When to Cache**:
    - UI files (zUI.*.zolo) after loading
    - Config files (zConfig.*.zolo) after parsing
...
**Caching with Mtime Checking**:
    >>> cache.set("ui_main", ui_data, filepath="/app/zUI.main.zolo")
    >>> # Later, if file changes, get() returns None and invalidates
    >>> data = cache.get("ui_main", filepath="/app/zUI.main.zolo")
...
Layer 1, Position 6 (zLoader - Tier 2 Cache Implementations)
    ...
    - Tier 2: Cache Implementations ← THIS MODULE
        ...
        - PythonModuleCache (Python module instances)
...
- loader_cache_python_module.py: Python module instance cache
...
from zOS import os, time, OrderedDict, Any, Dict, Optional
from zOS.L1_Foundation.a_zConfig.zConfig_modules import SESSION_KEY_ZCACHE, ZCACHE_KEY_SYSTEM
...
# Log Prefixes (concise format)
LOG_PREFIX_MISS: str = "[Cache] MISS:"
LOG_PREFIX_HIT: str = "[Cache] HIT:"
LOG_PREFIX_STALE: str = "[Cache] STALE:"
LOG_PREFIX_INVALID: str = "[Cache] INVALID:"
LOG_PREFIX_SET: str = "[Cache] SET:"
LOG_PREFIX_EVICT: str = "[Cache] EVICT:"
LOG_PREFIX_INVALIDATE: str = "[Cache] INVALIDATE:"
LOG_PREFIX_CLEAR: str = "[Cache] CLEAR:"
...
class SystemCache:
    ...
    def get_metadata(self, key: str) -> Optional[Dict[str, Any]]:
        """
        Get cache entry metadata without marking as accessed.
        
        This method retrieves metadata (mtime, cached_at, hits, filepath) without
        triggering cache hit counters or LRU reordering. Useful for HTTP cache
        validation where we need mtime but don't want to affect cache statistics.
        
        Args:
            key (str): Cache key to query
        
        Returns:
            Optional[Dict[str, Any]]: Metadata dict with keys:
                - mtime (float): File modification time (if tracked)
                - cached_at (float): When entry was cached
                - accessed_at (float): Last access time
                - hits (int): Number of cache hits
                - filepath (str): Source file path (if tracked)
                Returns None if key not found.
        
        Examples:
            >>> metadata = cache.get_metadata("parsed:/path/to/file.zolo")
            >>> if metadata and 'mtime' in metadata:
            ...     etag = generate_etag(mtime=metadata['mtime'])
        
        Note:
            Unlike get(), this method does NOT:
            - Increment hit counter
            - Update accessed_at timestamp
            - Move entry to end of LRU queue
            - Check file freshness (returns cached mtime as-is)
        """
        try:
            cache = self._cache
            ...
            # Return metadata without modifying entry
            metadata = {
                ENTRY_KEY_CACHED_AT: entry.get(ENTRY_KEY_CACHED_AT),
                ENTRY_KEY_ACCESSED_AT: entry.get(ENTRY_KEY_ACCESSED_AT),
                ENTRY_KEY_HITS: entry.get(ENTRY_KEY_HITS, 0)
            }

            # Add optional fields if present
            if ENTRY_KEY_MTIME in entry:
                metadata[ENTRY_KEY_MTIME] = entry[ENTRY_KEY_MTIME]

            if ENTRY_KEY_FILEPATH in entry:
                metadata[ENTRY_KEY_FILEPATH] = entry[ENTRY_KEY_FILEPATH]

            return metadata

        except Exception as e:
            self.logger.debug(LOG_PREFIX_ERROR + " get_metadata %s - %s", key, e)
            return None
