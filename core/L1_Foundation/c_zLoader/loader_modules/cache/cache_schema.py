# zOS/core/L1_Foundation/c_zLoader/loader_modules/cache/cache_schema.py
...
from zOS import time, Any, Dict, List, Optional
from zOS.L1_Foundation.a_zConfig.zConfig_modules import SESSION_KEY_ZCACHE, ZCACHE_KEY_SCHEMA
...
class SchemaCache:
    ...
    **Why Dual Storage?**
    ...
    def savepoint(self, alias_name: str, name: str) -> None:
        """
        Create a nested savepoint on the alias's open transaction.

        Delegates to the same adapter/connection the transaction holds, so an
        inner failure can unwind to ``name`` without aborting the outer commit.
        No-op (logged) if the alias has no connection.
        """
        if alias_name in self.connections:
            try:
                self.connections[alias_name].adapter.savepoint(name)
                self.logger.framework.debug(f"{LOG_PREFIX_TXN} Savepoint '{name}' on ${alias_name}")
            except Exception as e:
                self.logger.warning(f"{LOG_PREFIX_TXN} Error creating savepoint '{name}' on ${alias_name}: {e}")
        else:
            self.logger.warning(f"{LOG_PREFIX_TXN} Cannot savepoint - no connection for ${alias_name}")

    def release_savepoint(self, alias_name: str, name: str) -> None:
        """Release (merge) a savepoint on the alias — its work stays committed-in-txn."""
        if alias_name in self.connections:
            try:
                self.connections[alias_name].adapter.release_savepoint(name)
                self.logger.framework.debug(f"{LOG_PREFIX_OK} Savepoint '{name}' released on ${alias_name}")
            except Exception as e:
                self.logger.warning(f"{LOG_PREFIX_OK} Error releasing savepoint '{name}' on ${alias_name}: {e}")
        else:
            self.logger.warning(f"{LOG_PREFIX_OK} Cannot release savepoint - no connection for ${alias_name}")

    def rollback_to_savepoint(self, alias_name: str, name: str) -> None:
        """Undo work since a savepoint on the alias, keeping the outer transaction alive."""
        if alias_name in self.connections:
            try:
                self.connections[alias_name].adapter.rollback_to_savepoint(name)
                self.logger.warning(f"{LOG_PREFIX_ROLLBACK} Rolled back to savepoint '{name}' on ${alias_name}")
            except Exception as e:
                self.logger.warning(f"{LOG_PREFIX_ROLLBACK} Error rolling back to savepoint '{name}' on ${alias_name}: {e}")
        else:
            self.logger.warning(f"{LOG_PREFIX_ROLLBACK} Cannot rollback to savepoint - no connection for ${alias_name}")
