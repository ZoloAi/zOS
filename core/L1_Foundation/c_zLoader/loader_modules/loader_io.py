# zOS/core/L1_Foundation/c_zLoader/loader_modules/loader_io.py
...
--------------
**Used By**:
    - zOS/subsystems/zLoader/zLoader.py (Line 59)
...
**Basic Usage** (without display feedback):
    ...
    >>> logger = get_logger()
    >>> content = load_file_raw("/path/to/file.zolo", logger)
    >>> print(len(content))
    1234
...
**With Display Feedback** (zCLI/Bifrost modes):
    ...
    >>> logger = get_logger()
    >>> display = zos.display
    >>> content = load_file_raw("/path/to/file.zolo", logger, display)
    # Displays: "Reading" with SUBLOADER color in zCLI/Bifrost

**Error Handling**:
    >>> try:
    ...     content = load_file_raw("/nonexistent.zolo", logger)
    ... except RuntimeError as e:
    ...     print(f"Error: {e}")
    Error: Unable to load zFile (not found): /nonexistent.zolo
...
External:
    ...
    - zOS typing imports: Any, Optional (for type hints)
...
def load_file_raw(full_path: str, logger: Any, display: Optional[Any] = None) -> str:
    ...
    Args:
        ...
        display (Optional[Any], optional): Display instance for visual feedback
            in zCLI/Bifrost modes. If provided, shows "Reading" message
    ...
    Examples:
        ...
        Load a file with display feedback (zCLI/Bifrost modes):
            >>> logger = get_logger()
            >>> display = zos.display
            >>> content = load_file_raw("/app/ui.zolo", logger, display)
        ...
        Handle file not found error:
            ...
            ... except RuntimeError as e:
            ...     print(e)
            Unable to load zFile (not found): /missing.zolo
    ...
    if display:
        display.zDeclare(MSG_READING, color=COLOR_SUBLOADER, indent=INDENT_SECONDARY, style=STYLE_TILDE)
