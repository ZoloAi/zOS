# zOS/core/L1_Foundation/a_zConfig/zConfig_modules/helpers/detectors/ide.py
"""IDE and text editor detection for zCLI machine configuration."""

from zOS import os, platform, shutil, Optional
...
def detect_ide(log_level: Optional[str] = None, is_production: bool = False) -> str:
    ...
    for var in IDE_ENV_VARS:
        ...
        if ide_env:
            _log_config(f"IDE from env var {var}: {ide_env}", verbose=log_level is not None)
            return ide_env
    ...
    for ide in CLASSIC_IDES:
        if shutil.which(ide):
            _log_config(f"Found classic IDE: {ide}", verbose=log_level is not None)
            return ide

    # macOS-specific: Check for Xcode
    if platform.system() == "Darwin":
        if shutil.which("xed"):  # Xcode command-line tool
            _log_config("Found Xcode (xed)", verbose=log_level is not None)
            return "xed"

    # Fallback to simple editors
    for editor in SIMPLE_EDITORS:
        if shutil.which(editor):
            _log_config(f"Falling back to simple editor: {editor}", verbose=log_level is not None)
            return editor

    # Final fallback
    _log_config(f"Using final fallback: {FALLBACK_EDITOR}", verbose=log_level is not None)
