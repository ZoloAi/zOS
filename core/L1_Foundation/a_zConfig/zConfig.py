...
from .zConfig_modules.machine.config_resource_limits import ResourceLimits
...
class zConfig:
    ...
    def __init__(self, zos: Any, zSpark_obj: Optional[Dict[str, Any]] = None, verbose: bool = False) -> None:
        ...
        # Validate zOS instance, pre zSession creation
        ...
        self.sys_paths = zConfigPaths(zSpark_obj=zSpark_obj, verbose=verbose)
        ...
        self.machine = MachineConfig(self.sys_paths, verbose=verbose)
        ...
        self.environment = EnvironmentConfig(self.sys_paths, zSpark_obj, verbose=verbose)
        ...
        self.session = SessionConfig(self.machine, self.environment, zos, zSpark_obj, zconfig=self, verbose=verbose)

        # Create session and attach to zos instance
        session_data = self.session.create_session()
        zos.session = session_data
        ...
        zos.logger = session_logger

        # Log initial message with configured level (framework logger for internal init)
        zos.logger.framework.debug("Logger initialized at level: %s", session_logger.log_level)

        # Initialize centralized traceback utility
        # Import inline to avoid circular dependency (zTraceback imports zConfig types)
        from zSys.errors import zTraceback  # pylint: disable=import-error
        zos.zTraceback = zTraceback(logger=zos.logger, zos=zos)
        # NOTE: the interactive global sys.excepthook ("zTraceback mode") was retired.
        # zTraceback is now the error SHAPING CORE behind the `!` modifier only.

        # Initialize WebSocket configuration (uses environment config)
        self.websocket = WebSocketConfig(self.environment, zos, zos.logger, verbose=verbose)

        # Initialize HTTP Server configuration (optional feature)
        self.http_server = HttpServerConfig(zSpark_obj or {}, zos.logger, verbose=verbose)

        # Initialize zRaven test configuration (optional feature)
        self.raven = zRavenConfig(zSpark_obj or {}, zos.logger)

        # Initialize storage path manager (Phase 1.5: Storage Architecture)
        from .zConfig_modules.paths.config_storage_paths import StoragePathManager
        ...
        # Show in Development mode or when verbose=True
    ...
    def get_environment_struct(self, key: str, default: Any = None) -> Any:
        """Get a STRUCTURED zEnv value (nested dict/list) by key.

        zEnv nested blocks are stored in os.environ as JSON strings; this returns
        them parsed (dict/list) instead of the raw string. Use for app-declared
        manifests (nested dict/list zEnv blocks) that subsystems consume
        structurally. Parsing stays in the config layer — callers never import json.
        """
        return self.environment.get_struct(key, default)
    ...
    def create_logger(self, session_data: Dict[str, Any]) -> LoggerConfig:
        ...
        """
        return LoggerConfig(self.environment, self.zos, session_data, verbose=self._verbose)
    ...
    def persistence(self) -> ConfigPersistence:
        ...
        if not hasattr(self, '_persistence'):
            self._persistence = ConfigPersistence(self.machine, self.environment, self.sys_paths, self.zos)  # pylint: disable=attribute-defined-outside-init
