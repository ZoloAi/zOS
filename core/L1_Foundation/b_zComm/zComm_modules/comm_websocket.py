# zOS/core/L1_Foundation/b_zComm/zComm_modules/comm_websocket.py
"""
...
from zOS import (
    asyncio, Any, Optional, Dict, Callable, Set,
    ws_serve, WebSocketServerProtocol, logging, ssl
)
...
class WebSocketHandshakeFilter(logging.Filter):
    ...
    Architecture:
        ...
        - Applied only to the 'websockets' logger (doesn't affect zOS logging)
...
class WebSocketServer:
    ...
    def _custom_exception_handler(self, _loop: Any, context: Dict[str, Any]) -> None:
        """
        ...
        Args:
            _loop: Asyncio event loop (unused, required by asyncio API)
    ...
    def start(
        ...
        Uses zConfig defaults if not specified (respects 5-layer hierarchy).
        zOS handles asyncio and graceful shutdown internally.
    ...
    async def _handle_client(self, websocket: WebSocketServerProtocol) -> None:
        ...
        if self.config.require_auth:
            ...
            if not self.auth.check_connection_limit():
                await websocket.close(code=WS_CLOSE_CODE_POLICY_VIOLATION, reason=WS_REASON_MAX_CONNECTIONS)
                return

            # 2. Validate origin (CORS/CSRF protection)
            if not self.auth.validate_origin(websocket):
                await websocket.close(code=WS_CLOSE_CODE_POLICY_VIOLATION, reason=WS_REASON_INVALID_ORIGIN)
                return
            ...
            if not token:
                ...
                await websocket.close(code=WS_CLOSE_CODE_POLICY_VIOLATION, reason=WS_REASON_AUTH_REQUIRED)
                return

            if not self.auth.validate_token(token):
                self.logger.warning(f"{_LOG_PREFIX} Invalid token from {client_addr}")
                await websocket.close(code=WS_CLOSE_CODE_POLICY_VIOLATION, reason=WS_REASON_INVALID_TOKEN)
                return
