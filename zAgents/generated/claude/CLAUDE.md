...
topic_refs:
    ...
    Dynamic content (zLoom) — the % sigil weaves live/repeated/computed content pre-render: %token values (zSpool/%session…), %name: patterns (zShuttle loops a list, zKnot computes), zDye pipes → read ~/.claude/zolo/dynamic_content.md
