<!-- cursor: description="zServer — flip one zSpark key and the terminal app serves the web: zViews/ → URLs, mounts, guarded error pages, caching, live reload, zAPI. Declared, never plumbed" globs="**/zServer.*.zolo,**/zEnv.*.zolo,**/zSpark.*.zolo" alwaysApply=false -->
